...
# 第 1.5 版 V1.5（打磨版 / Polish）—— 补齐 V1 已知缺口

> 目标：让 V1 从「功能正确」升级为「能看能用」的完整产品。以下两块不在原 spec，
> 为「能看能用 + 过审观感」补充。分类：UI 设计体系 / 工具真实交互。

## V1.5 · 统一 UI 设计体系

- [ ] U1. 设计令牌与全局样式基线
  - 新建全局 CSS 设计令牌（颜色/间距/圆角/阴影/字体），确立 CSDN 风格视觉基调（品牌红、卡片、留白）
  - 提供全局重置样式与排版基线；不引入重型 UI 框架依赖（纯 CSS/CSS Modules）
  - _缺口：统一 UI 设计_

- [ ] U2. 通用 UI 组件库
  - 实现共享组件：Button、Input、Card、PageHeader、Badge/Tag、Modal/Dialog、EmptyState、Spinner
  - 统一交互态（hover/disabled/focus）与可访问性
  - _缺口：统一 UI 设计_

- [ ] U3. 应用外壳与导航美化
  - 顶部导航栏（伪装成 CSDN 顶栏）、受保护布局、便签侧栏 dock 的视觉落地
  - 登录/注册页视觉设计
  - _缺口：统一 UI 设计_

- [ ] U4. 各功能页视觉落地
  - 文档库（卡片/列表 + 上传区）、工具页（卡片网格 + 职业选择器）、合规页排版套用新组件与令牌
  - _缺口：统一 UI 设计_

## V1.5 · 工具真实交互

- [ ] T1. 工具运行时框架（ToolRunner + 注册表）
  - 建立 slug → 工具组件 的前端注册表；点击「启动」在 Modal 中渲染对应工具组件（替换现有仅返回可用状态的行为）
  - _缺口：工具真实交互_

- [ ] T2. 时间/计算类工具
  - 下班倒计时、计时器（含番茄钟）、日期计算、计算器、汇率换算
  - _缺口：工具真实交互_

- [ ] T3. 开发者类工具
  - JSON 格式化、时间戳转换、正则测试、单位/格式转换、颜色转换
  - _缺口：工具真实交互_

- [ ] T4. 文本类工具
  - 文本处理（大小写/去重/去空行/排序）、字数统计
  - _缺口：工具真实交互_
