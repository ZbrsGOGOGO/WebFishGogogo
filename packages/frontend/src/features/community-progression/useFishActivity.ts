...
export const isFishGamePath = (path: string): boolean => /^\/(?:games\/(?!rooms\/?$)[^/]+|tower-defense(?:\/|$))/.test(path);
...
export function useFishActivity(): void {
  ...
  useEffect(() => {
    ...
    const tick = async (): Promise<void> => {
      ...
      busy = true;
      try {
        const progress = await communityProgressionApi.heartbeat({ tabId, sequence: ++sequence, mode: !active ? 'pause' : isFishGamePath(path.current) ? 'game' : 'browse' }, controller.signal);
        paused = !active;
