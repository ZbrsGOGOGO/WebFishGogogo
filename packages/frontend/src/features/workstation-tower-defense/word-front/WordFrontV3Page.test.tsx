...
import { replayWordFrontV3, type WordFrontV3Action } from '@stealth-reader/shared';
...
import { WordFrontV3Page } from './WordFrontV3Page';
...
function page(): void { render(<MemoryRouter><WordFrontV3Page /></MemoryRouter>); }

describe('WordFrontV3Page', () => {
  beforeEach(() => {
    ...
    vi.mocked(startArcadeRun).mockResolvedValue({ runId: 'run-v2', gameKey: 'word_story_v3', rulesVersion: 3, chapter: 1, seed: 17,
      startedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 2 * 60 * 60_000).toISOString() });
    vi.mocked(getArcadeLeaderboard).mockResolvedValue({ gameKey: 'word_story_v3', formulaVersion: 'word-front-v3', items: [] });
    vi.mocked(finishArcadeRun).mockResolvedValue({ gameKey: 'word_story_v3', score: 0, bestScore: 0, isPersonalBest: false, rank: 1 });
  });
  ...
  it('keeps the play board before recruitment/ranking on mobile and gives cards distinct spoken positions', async () => {
    ...
    const css = readFileSync(resolve(process.cwd(), 'src/features/workstation-tower-defense/word-front/WordFrontV3Page.module.css'), 'utf8');
  ...
  it('shows six chapters, preserved v1/v2 routes, a separate v3 leaderboard and explicitly requests rules v3', async () => {
    page();
    expect(screen.getByRole('link', { name: '初版草稿' })).toHaveAttribute('href', '/tower-defense/word-front/legacy');
    expect(screen.getByRole('link', { name: '旧版六章' })).toHaveAttribute('href', '/tower-defense/word-front/v2');
    ...
    expect(getArcadeLeaderboard).toHaveBeenCalledWith('word_story_v3');
    fireEvent.click(screen.getByRole('button', { name: '招募五张 · 10 点' }));
    await waitFor(() => expect(within(screen.getByRole('group', { name: '手中字卡' })).getAllByRole('button').length).toBeGreaterThanOrEqual(4));
    expect(startArcadeRun).toHaveBeenCalledExactlyOnceWith('word_story_v3', 3, 1);
  ...
  it('keeps endless mode and its leaderboard isolated from story', async () => {
    vi.mocked(startArcadeRun).mockResolvedValue({ runId: 'run-endless-v2', gameKey: 'word_endless_v3', rulesVersion: 3, chapter: 1, seed: 17,
      startedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 2 * 60 * 60_000).toISOString() });
    ...
    await waitFor(() => expect(getArcadeLeaderboard).toHaveBeenCalledWith('word_endless_v3'));
    fireEvent.click(screen.getByRole('button', { name: '招募五张 · 10 点' }));
    await waitFor(() => expect(startArcadeRun).toHaveBeenCalledWith('word_endless_v3', 3, 1));
  });
  ...
  it('uses the chapter bound to a reused online run and ignores a late response after account switch', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: '招募五张 · 10 点' }));
    expect(startArcadeRun).toHaveBeenCalledWith('word_story_v3', 3, 2);
    await act(async () => resolveRun({ runId: 'old-chapter', gameKey: 'word_story_v3', rulesVersion: 3, chapter: 1, seed: 17,
      startedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 2 * 60 * 60_000).toISOString() }));
    ...
    await act(async () => resolveRun({ runId: 'late-a', gameKey: 'word_story_v3', rulesVersion: 3, chapter: 1, seed: 18,
      startedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 2 * 60 * 60_000).toISOString() }));
    expect(screen.getByText('暂无字卡，先招募一次。')).toBeInTheDocument();
    expect(window.localStorage.getItem('momo.word-front.v3.member-b.story')).not.toContain('"hand":["');
  });

  it('keeps a v3 game local when an old API ignores the version request', async () => {
    vi.mocked(startArcadeRun).mockResolvedValue({ runId: 'old-api', gameKey: 'word_story_v3', seed: 17,
      startedAt: new Date().toISOString(), expiresAt: new Date(Date.now() + 2 * 60 * 60_000).toISOString() });
    page();
    fireEvent.click(screen.getByRole('button', { name: '招募五张 · 10 点' }));
  ...
  it('keeps a v3 game local when an old API rejects the new game key', async () => {
  ...
  it('does not promise a ranked game when a reused run has under five minutes left', async () => {
    vi.mocked(startArcadeRun).mockResolvedValue({ runId: 'nearly-expired', gameKey: 'word_story_v3', rulesVersion: 3, chapter: 1, seed: 17,
  ...
  it('submits a real terminal UI trace that the v3 engine replays exactly', async () => {
    page(); fireEvent.click(screen.getByRole('button', { name: '招募五张 · 10 点' }));
    await waitFor(() => expect(within(screen.getByRole('group', { name: '手中字卡' })).getAllByRole('button').length).toBeGreaterThanOrEqual(4));
    const cards = within(screen.getByRole('group', { name: '手中字卡' })).getAllByRole('button');
    fireEvent.click(cards[0]!); fireEvent.click(cards[1]!);
    ...
    const replay = replayWordFrontV3('story', 1, 17, metrics.actions as WordFrontV3Action[], metrics.finishTick as number);
