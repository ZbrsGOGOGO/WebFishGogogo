...
function V4Run({ publicId, mode, onModeChange }: { publicId: string; mode: WordFrontMode; onModeChange: (mode: WordFrontMode) => void }): JSX.Element {
  ...
  return <main className={styles.page}>
    ...
    </aside><aside className={styles.ranking} aria-label="赵云救阿斗排行与规则"><ArcadeLeaderboard gameKey={mode==='story'?'word_story_v4':'word_endless_v4'} refreshKey={arcade.revision} /><button onClick={()=>setRules(value=>!value)}>{rules?'收起规则':'完整规则'}</button>{rules?<div><p>V4 与旧榜分开；服务器用种子和动作轨迹完整重放后才收分。</p><p>只使用局内包子与金币，不扣办公币、不接付费。页面切走或失焦会暂停单机画面。</p><p>原型依据 MIT 仓库的玩法数据重新实现；账号、权限和结算均为本站代码。</p></div>:null}{arcade.notice?<p className={styles.local}>{arcade.notice}</p>:null}{!eligible&&state.drawCount>0?<p className={styles.local}>当前为本机演练，不提交排行榜。</p>:null}</aside></div>
  </main>;
}
