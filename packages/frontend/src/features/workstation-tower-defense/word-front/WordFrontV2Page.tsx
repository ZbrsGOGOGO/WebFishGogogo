...
import {
  applyWordFrontV2Action, createWordFrontV2State, stepWordFrontV2,
  wordFrontV2DrawCost, wordFrontV2HeroForLetters, wordFrontV2Terrain, wordFrontV2UnitForLetter,
  WORD_FRONT_V2_CHAPTERS, WORD_FRONT_V2_HEIGHT, WORD_FRONT_V2_UNITS, WORD_FRONT_V2_WIDTH,
  type WordFrontMode, type WordFrontV2Action, type WordFrontV2State, type WordFrontV2UnitKind,
...
import styles from './WordFrontV2Page.module.css';
...
function safeDraft(key: string, mode: WordFrontMode): WordFrontV2State | null {
  try {
    const raw = window.localStorage.getItem(key);
    if (!raw || raw.length > 128_000) return null;
    const state = JSON.parse(raw) as Partial<WordFrontV2State>;
    if (state.version !== 2 || state.mode !== mode || !['ready', 'running', 'won', 'lost'].includes(state.status ?? '') ||
      ...
      !state.units.every(unit => validInt(unit.slot, 0, 47) && validInt(unit.level, 1, 3) && Object.prototype.hasOwnProperty.call(WORD_FRONT_V2_UNITS, unit.kind)) ||
      new Set(state.units.map(unit => unit.slot)).size !== state.units.length || !Array.isArray(state.enemies) || state.enemies.length > 32 ||
      !state.enemies.every(enemy => validInt(enemy.id, 1, 1_000) && validInt(enemy.pathIndex, 0, 20) && validInt(enemy.hp, 1, 1_000) &&
        validInt(enemy.maxHp, 1, 1_000) && validInt(enemy.moveEvery, 1, 6) && Object.prototype.hasOwnProperty.call(ENEMIES, enemy.kind))) return null;
    return state as WordFrontV2State;
  } catch { return null; }
}

export function WordFrontV2Page(): JSX.Element {
  ...
  const adapter = useMemo<ArcadeAdapter>(() => ({ signedIn: phase === 'active', restoreSession,
    startRun: (gameKey, chapter) => startArcadeRun(gameKey, 2, chapter), finishRun: finishArcadeRun, getLeaderboard: getArcadeLeaderboard }), [phase, restoreSession]);
  ...
  return <ArcadeAdapterProvider adapter={adapter}><WordFrontV2Run key={`${publicId}:${mode}`} publicId={publicId} mode={mode} onModeChange={setMode} /></ArcadeAdapterProvider>;
}

function WordFrontV2Run({ publicId, mode, onModeChange }: { publicId: string; mode: WordFrontMode; onModeChange: (mode: WordFrontMode) => void }): JSX.Element {
  const storageKey = `momo.word-front.v2.${publicId}.${mode}`;
  const [state, setState] = useState<WordFrontV2State>(() => safeDraft(storageKey, mode) ?? createWordFrontV2State(mode));
  const stateRef = useRef(state), actions = useRef<WordFrontV2Action[]>([]);
  ...
  const gameOwner = useRef(`word-front-v2-${publicId}-${mode}`).current;
  const arcade = useArcadeRun(mode === 'story' ? 'word_story_v2' : 'word_endless_v2');
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { const next = stepWordFrontV2(stateRef.current); stateRef.current = next; setState(next); }, 850);
  ...
  const chosenKind: WordFrontV2UnitKind | null = selectedCards.length === 2
    ? wordFrontV2HeroForLetters(state.hand[selectedCards[0]!] ?? '', state.hand[selectedCards[1]!] ?? '')
    : selectedCards.length === 1 ? wordFrontV2UnitForLetter(state.hand[selectedCards[0]!] ?? '') : null;
  const map = WORD_FRONT_V2_CHAPTERS[(mode === 'endless' ? 6 : state.chapter) - 1]!;
  const path = map.path as readonly number[];
  const active = state.status === 'running';
  function set(next: WordFrontV2State): void { stateRef.current = next; setState(next); }
  function apply(action: WordFrontV2Action): boolean {
    const next = applyWordFrontV2Action(stateRef.current, action);
  ...
  async function recruit(): Promise<void> {
    ...
    if (!sessionStarted.current && stateRef.current.drawCount === 0) {
      ...
      const gameKey = mode === 'story' ? 'word_story_v2' : 'word_endless_v2';
      // A previous API ignores rulesVersion. It must never receive a v2 trace.
      const verified = run?.gameKey === gameKey && run.rulesVersion === 2 && validInt(run.seed, 0, 0xffff_ffff) &&
      ...
      const base = createWordFrontV2State(mode, boundChapter, verified ? run.seed! : stateRef.current.seed);
      const action: WordFrontV2Action = { tick: 0, type: 'recruit' };
      const next = applyWordFrontV2Action(base, action);
    ...
    if (!apply({ tick: stateRef.current.tick, type: 'recruit' })) set({ ...stateRef.current, message: `局内经费不足，下次招募需 ${wordFrontV2DrawCost(stateRef.current.drawCount)} 点。` });
  }
  function chooseCell(slot: number): void {
    const terrain = wordFrontV2Terrain(stateRef.current, slot);
    if (terrain === 'waste') {
    ...
    if (chosenKind && !occupied) {
      const action: WordFrontV2Action = selectedCards.length === 2
  ...
  function reset(): void {
    ...
    requestId.current += 1;
    set(createWordFrontV2State(mode, chapter, (Date.now() ^ Math.floor(Math.random() * 0x7fffffff)) >>> 0));
  ...
  return <main className={styles.page}>
    <header className={styles.header}><div><span className={styles.kicker}>WORKSHEET / WORD FRONT 2</span><h1>文字战线 · 赵云救阿斗</h1><p>六章护送、文字编队与路线布防；页面像一份安静的工作表。</p></div><nav><Link to="/tower-defense">原工位塔防</Link><Link to="/tower-defense/word-front/legacy">旧版草稿</Link><Link to="/games">小游戏专区</Link></nav></header>
    <div className={styles.layout}>
      <section className={styles.mainPanel} aria-label="文字战线对局">
        <div className={styles.toolbar}><div className={styles.modes} role="group" aria-label="玩法模式">{(Object.keys(MODES) as WordFrontMode[]).map(key => <button key={key} type="button" data-active={mode === key} aria-pressed={mode === key} onClick={() => changeMode(key)}>{MODES[key]}</button>)}</div>
          {mode === 'story' ? <label>章节<select value={chapter} disabled={active || starting || state.drawCount > 0} onChange={event => { const value = Number(event.target.value); setChapter(value); set(createWordFrontV2State(mode, value)); setPaused(true); setEligible(false); actions.current = []; sessionStarted.current = false; }}>{WORD_FRONT_V2_CHAPTERS.map(item => <option key={item.id} value={item.id}>{item.id}. {item.name}</option>)}</select></label> : <span>无尽 · 30 波 / 每 10 波首领</span>}
        </div>
        <p className={styles.chapterDetail}>{mode === 'story' ? `${state.chapter}. ${map.name} · ${state.chapter + 2} 波，末波首领` : '大老板巡场路线，波数递增；不扣账号办公币。'}</p>
        <div className={styles.metrics} aria-label="对局数据"><span>核心<strong>{state.coreHp}/5</strong></span><span>波次<strong>{state.wave || '待命'}</strong></span><span>击败<strong>{state.kills}</strong></span><span>得分<strong>{state.score}</strong></span><span>经费<strong>{state.credits}/40</strong></span><span>局内金币<strong>{state.gold}</strong></span><span>铲子<strong>{state.shovels}</strong></span></div>
        <div className={styles.boardViewport}><div className={styles.board} role="group" aria-label="文字战线地图">{Array.from({ length: WORD_FRONT_V2_WIDTH * WORD_FRONT_V2_HEIGHT }, (_, slot) => {
          const terrain = wordFrontV2Terrain(state, slot), pathIndex = path.indexOf(slot);
          const unit = state.units.find(item => item.slot === slot), enemies = pathIndex >= 0 ? state.enemies.filter(item => item.pathIndex === pathIndex) : [];
          const row = Math.floor(slot / 8) + 1, column = slot % 8 + 1;
          const label = `${row}行${column}列，${unit ? `${WORD_FRONT_V2_UNITS[unit.kind].name}${unit.level}阶` : terrain === 'path' ? '路线' : terrain === 'slow' ? '减速地毯' : terrain === 'waste' ? '未开垦荒地' : terrain === 'obstacle' ? '障碍' : terrain === 'buff' ? '增益工位' : '空白工位'}${enemies.length ? `，${enemies.length}名来客` : ''}`;
          return <button key={slot} type="button" className={styles.cell} data-terrain={terrain} data-unit={unit?.kind ?? ''} data-selected={selectedUnit === slot} data-front={pathIndex >= 0 && pathIndex < routeProgress} aria-pressed={unit ? selectedUnit === slot : undefined} aria-label={label} disabled={terrain === 'path' || terrain === 'slow' || terrain === 'obstacle'} onClick={() => chooseCell(slot)}>
            {unit ? <span className={styles.unit}><b>{WORD_FRONT_V2_UNITS[unit.kind].glyph}</b><small>{WORD_FRONT_V2_UNITS[unit.kind].name} · {unit.level}阶</small></span> : pathIndex === 0 ? <span className={styles.pathLabel}>入口</span> : pathIndex === path.length - 1 ? <span className={styles.pathLabel}>鱼</span> : terrain === 'waste' ? <span className={styles.terrainGlyph}>锁</span> : terrain === 'obstacle' ? <span className={styles.terrainGlyph}>柜</span> : terrain === 'buff' ? <span className={styles.terrainGlyph}>充</span> : terrain === 'slow' ? <span className={styles.pathLabel}>缓</span> : terrain === 'path' ? <span className={styles.pathDot}>·</span> : <span className={styles.emptyCell}>+</span>}
        ...
        <p className={styles.note}>窄屏可横向滑动地图。字卡选一个基础单位，或选互补双字职业，然后点空工位部署；点两位同角色同阶单位可合并。白色荒地需铲子解封，金色工位增加输出，蓝色地毯拖慢来客。</p>
      ...
      <aside className={styles.sidePanel} aria-label="招募与编队"><span className={styles.kicker}>01 / RECRUIT</span><h2>字卡工作台</h2><p>招募五张；10 点起，每次加 2。首抽保底一组职业双字，后续按基础字 90%、职业字 8%、铲子 2% 抽取。未过期的在线对局重新开始会复用同一种子，不可重抽开局。</p>
        <button className={styles.recruit} type="button" disabled={starting || state.status === 'won' || state.status === 'lost'} onClick={() => { void recruit(); }}>{starting ? '正在建立可验证对局…' : `招募五张 · ${wordFrontV2DrawCost(state.drawCount)} 点`}</button>
        ...
        <p className={styles.combo}>{chosenKind ? <>已选 <strong>{WORD_FRONT_V2_UNITS[chosenKind].name}</strong>，点击空工位部署。</> : selectedCards.length ? '基础单位单字可部署；职业需要互补双字。' : '选择单字或互补双字。'}</p>
        <div className={styles.roster}><h3>角色与装备字</h3>{Object.entries(WORD_FRONT_V2_UNITS).map(([key, value]) => <div key={key}><b>{value.glyph}</b><span>{value.name}</span><small>{value.role}</small></div>)}</div>
      ...
      <aside className={styles.rankingPanel} aria-label="文字战线成绩与规则"><h3>新版独立成绩</h3><p>新版剧情与无尽分榜，旧版成绩留在“旧版草稿”入口的旧榜；版本间不混排。服务端逐动作回放核验，局内资源不接账号办公币，也不发放奖励。</p>{onlineExpiresAt && eligible ? <p role="status">本局在线成绩有效至 {new Date(onlineExpiresAt).toLocaleString('zh-CN')}；过期后只保留本机草稿。重开若旧对局尚有充裕时间，会复用原种子。</p> : null}{state.drawCount > 0 && !eligible ? <p role="status">当前是恢复或离线草稿，不提交排行榜；新开局获取 v2 服务端规则种子后才能参榜。</p> : null}{arcade.notice ? <p role="status">{arcade.notice}</p> : null}<ArcadeLeaderboard gameKey={mode === 'story' ? 'word_story_v2' : 'word_endless_v2'} refreshKey={arcade.revision} />
        <button className={styles.rulesToggle} type="button" aria-expanded={rulesOpen} onClick={() => setRulesOpen(!rulesOpen)}>{rulesOpen ? '收起规则' : '展开规则'}</button>{rulesOpen ? <ol><li>旧版草稿在“旧版草稿”入口继续游玩，不会自动转换为新版规则。</li><li>五个基础字可单字部署；职业字必须组成赵云、关羽、张飞、黄忠或马超。</li><li>局内经费上限 40，只由击败来客和清波补给产生；没有被动产出。</li><li>铲子在本局开垦荒地；金色工位增伤，蓝色地毯减速。</li><li>本页不提供真人房间对战、局外商店或账号付费。</li></ol> : null}
      </aside>
    </div>
  </main>;
}
