...
import { buyWordFrontCosmetic, finishArcadeRun, getArcadeLeaderboard, getWordFrontProgress, startArcadeRun } from '../../../api/community-arcade';
...
vi.mock('../../../api/community-arcade', () => ({ startArcadeRun: vi.fn(), finishArcadeRun: vi.fn(), getArcadeLeaderboard: vi.fn(), getWordFrontProgress: vi.fn(), buyWordFrontCosmetic: vi.fn() }));
...
describe('WordFrontV4Page',()=>{
  beforeEach(()=>{vi.resetAllMocks();localStorage.clear();resetCommunityAuthStoreForTests();useCommunityAuthStore.setState({phase:'active',user,sessionReady:true,restoreSession:vi.fn()});
    ...
    vi.mocked(getWordFrontProgress).mockResolvedValue({merit:180,wins:3,losses:1,dailyEarned:24,dailyCap:100,unlocks:['changban_badge'],shop:[{id:'ledger_theme',name:'台账主题',cost:80,description:'低调配色',owned:false}]});
    vi.mocked(buyWordFrontCosmetic).mockResolvedValue({merit:100,wins:3,losses:1,dailyEarned:24,dailyCap:100,unlocks:['changban_badge','ledger_theme'],shop:[{id:'ledger_theme',name:'台账主题',cost:80,description:'低调配色',owned:true}]});
  ...
  it('shows server merit and purchases a cosmetic without touching gameplay state',async()=>{page();expect(await screen.findByText('长坂徽记')).toBeVisible();fireEvent.click(screen.getByRole('button',{name:'台账主题 · 80'}));await waitFor(()=>expect(buyWordFrontCosmetic).toHaveBeenCalledExactlyOnceWith('ledger_theme'));expect(await screen.findByRole('button',{name:'已收藏 · 台账主题'})).toBeDisabled();});
});
