...
import { WORKSTATION_CHAPTERS, WORKSTATION_JOBS, WORKSTATION_PROMOTIONS, TOWER_DEFINITIONS, OFFICE_COLLECTION, type WorkstationCommand, type WorkstationJob, type WorkstationMode, type WorkstationOverview, type WorkstationTalents } from '@stealth-reader/shared';
...
export function WorkstationCampaignPage({ character }: { character?: WorkstationTowerDefenseCharacter }): JSX.Element {
  ...
  const [talents,setTalents] = useState<WorkstationTalents>({output:0,control:0,economy:0});
  ...
  useEffect(() => {
    ...
    void workstationApi.overview(controller.signal).then((next)=> { if (alive.current && generation === getCommunitySessionGeneration()) { setView(next);setTalents(next.profile.talents); } }).catch((err:unknown)=> { if (!controller.signal.aborted) setError(errorMessage(err)); });
  ...
  return <div className={styles.page} data-office-appearance={appearance?.id} style={appearance?{'--office-appearance':appearance.color} as React.CSSProperties:undefined}>
    ...
    {dashboard?<>
      ...
      <section className={styles.talents} aria-label="三系天赋"><h2>成长便签 · 三系天赋</h2><p>共 {profile.talentPoints} 点，免费重新分配，下局生效。出力每点 +4% 伤害；控制扩大脉冲并减冷却；经济每点 +8% 绿植收入。</p>{(['output','control','economy'] as const).map((key,index)=><label key={key}>{['出力','控制','经济'][index]}<input type="number" min="0" max="5" value={talents[key]} onChange={e=>setTalents(old=>({...old,[key]:Number(e.target.value)}))}/></label>)}<button type="button" disabled={active||busy||!view.writesEnabled} onClick={()=>{void mutate(()=>workstationApi.talents(talents));}}>保存天赋</button></section>
    ...
    </>:run?<WorkstationTowerDefensePage character={character} session={{state:run.state,pending:busy,onCommand:send,onRestart:()=>setDashboard(true)}}/>:null}
  </div>;
}
