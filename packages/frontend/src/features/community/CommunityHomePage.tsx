...
export function CommunityHomePage(): JSX.Element {
  ...
  const professionLabel = COMMUNITY_PROFESSIONS.find(
      ...
      <section className={styles.section} aria-labelledby="desk-title">
        ...
        <div className={styles.statusGrid}>
          <Link to={signedIn ? '/me' : '/login'}>
            ...
            <div><small>个人档案</small><strong>{signedIn ? profileSummary?.displayName : '保存你的成长'}</strong><p>{signedIn ? `社区职业：${professionLabel}` : '资料、好友和绿植跨设备保留'}</p></div>
          </Link>
          ...
          {COMMUNITY_FEATURE_FLAGS.friends ? (
            <Link to={signedIn ? '/friends' : '/login'}>
              ...
              <div><small>好友</small><strong>{signedIn && summaries.friends ? `${summaries.friends.pendingIncomingCount} 条待处理` : '找到熟悉的同事'}</strong><p>{summaryErrors.friends ?? '互相投喂、鼓励绿植，也可以分享塔防成绩'}</p></div>
            </Link>
          ) : null}
        </div>
      </section>
