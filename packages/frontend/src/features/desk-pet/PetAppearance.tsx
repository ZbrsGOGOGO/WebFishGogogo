...
export function PetAppearance({ prefs, preview = false, mood = null }: { prefs: PetPreferences; preview?: boolean; mood?: 'happy' | 'eating' | null }) {
  ...
  return <span className={styles.appearance} data-style={prefs.style} data-sleeping={prefs.sleeping} data-still={prefs.quiet || preview} style={{ '--pet-size': `${prefs.size}px` } as CSSProperties}>
    ...
    {!prefs.sleeping && mood ? <span className={styles.mood} data-mood={mood} aria-hidden="true">{mood === 'eating' ? '🍪' : '♡'}</span> : null}
  </span>;
}
