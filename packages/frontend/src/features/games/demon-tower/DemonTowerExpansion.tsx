...
type Props = { profile: DemonTowerProfileView; world: DemonTowerWorldView; disabled: boolean; onAction: (action: DemonTowerAction) => Promise<boolean>; onContinueBattle?: () => void; onSupplies?: () => void; onLoadout?: () => void };

/** Uses the same versioned, uncertain-result-aware action queue as ordinary exploration. */
export function DemonTowerExpansion({ profile, disabled, onAction, onContinueBattle, onSupplies, onLoadout }: Props): JSX.Element | null {
  ...
  return <div className={styles.stack}>
    ...
    <TowerPanel title="星级与稀有度工坊">
      <div className={styles.cultivationGuide} aria-label="工坊培养入口"><p><strong>升星看星数，突破升稀有度。</strong><small>本处处理残魂升星、武器稀有度突破与品质经验回收；品质 +N 的手动强化在装备技能，三者不是同一操作。</small></p>{onLoadout ? <button type="button" className={styles.button} onClick={onLoadout}>前往装备强化</button> : null}</div>
      <label className={styles.field}>选择已收录物品 <select aria-label="成长工坊物品" value={selected} onChange={event => setSelected(event.target.value)}>{[...profile.weapons, ...profile.skills].map(item => <option key={item.id} value={item.id}>{(item.id.startsWith('w') ? DEMON_TOWER_WEAPONS : DEMON_TOWER_SKILLS).find(entry => entry.id === item.id)?.name}</option>)}</select></label>
      {owned && definition ? <>
        ...
        <div className={styles.buttonRow}>
          <button className={styles.button} type="button" disabled={!can('star_up') || star >= 5 || profile.materials.soul < RULES.starCharmSoul} onClick={() => ask('残魂升星', `消耗40残魂，本次成功率${Math.round((RULES.starSuccess[star - 1] ?? 0) * 100)}%。失败保留星级和熟练度，不降星；成功后新星级熟练度从0开始。也可完全依靠使用熟练度免费升星。`, { kind: 'star_up', payload: { itemType: kind, itemId: owned.id } })}>残魂升星 · 40残魂</button>
