...
export function DemonTowerExpansion({ profile, disabled, onAction, selectedItem, onSelectItem, active = true, advancedEnabled = true }: Props): JSX.Element | null {
  ...
  return <div className={styles.stack}>
    <TowerPanel title={advanced ? '养成工坊 · 星级 / 稀有度 / 品质' : '养成工坊 · 基础品质强化'}>
      ...
      {owned && definition ? <>
        ...
        <div className={styles.buttonRow}>
          ...
          {value?.seasonal ? <button className={styles.button} type="button" disabled={!can('golden_star_up') || star >= 5 || value.seasonal.goldenScrolls < star} onClick={() => ask('黄金卷轴升星', `消耗${star}张黄金卷轴。失败不降星，累积祝福；${value.seasonal?.starFestival ? '本周为升星庆典，必定成功。' : '祝福达到门槛时必定成功。'}五星时觉醒神技效果。`, { kind: 'golden_star_up', payload: { itemType: kind, itemId: owned.id } })}>黄金卷轴升星 · {star}张</button> : null}
    ...
    {advanced && value?.seasonal ? <TowerPanel title="周期补给 · 签到 / 黄金卷轴">
      <p className={styles.muted}>黄金卷轴 {value.seasonal.goldenScrolls} · {value.seasonal.starFestival ? '本周升星庆典：卷轴升星必定成功' : '常规周：失败累积祝福，绝不降星'}。所有物品仅用残魂，不使用办公币或付费。</p>
      <div className={styles.buttonRow}>
        <button className={styles.primary} type="button" disabled={!can('daily_sign_in') || value.seasonal.signedIn} onClick={() => void onAction({ kind: 'daily_sign_in', payload: {} })}>{value.seasonal.signedIn ? '今日已签到' : '今日签到 · 卷轴×2 + 残魂×5'}</button>
      </div>
      <div className={styles.itemGrid}>{value.seasonal.offers.map(offer => <article className={styles.resource} key={offer.id}><strong>{offer.name}</strong><small>{offer.description} · {offer.period === 'daily' ? '每日' : '每周'} {offer.purchased}/{offer.limit}</small><button className={styles.button} type="button" disabled={!can('seasonal_purchase') || !offer.available || profile.materials.soul < offer.cost} onClick={() => ask(`兑换${offer.name}`, `消耗${offer.cost}绑定残魂，${offer.description}。不扣办公币，服务器会核验${offer.period === 'daily' ? '当日' : '当周'}限额。`, { kind: 'seasonal_purchase', payload: { offerId: offer.id } })}>{offer.available ? `${offer.cost}残魂兑换` : '本期已达上限'}</button></article>)}</div>
    </TowerPanel> : null}
