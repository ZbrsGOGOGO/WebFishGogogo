      ...
      <div className={compact.grid} aria-label={category === 'weapons' ? '本页武器' : '本页技能'}>
        {category === 'weapons' ? visibleWeapons.slice(pageStart, pageStart + PAGE_SIZE).map(weapon => {
          ...
          return <article className={compact.card} key={skill.id} aria-label={skill.name} data-selected={selected} data-owned={Boolean(owned)}>
            ...
            <p className={compact.stats}>{owned ? <>{owned.star !== undefined ? <span>★ {owned.star}/5</span> : null}{owned.star !== undefined && owned.star >= 5 ? <span>熟练度已满（不再累积）</span> : owned.favor !== undefined ? <span>熟练度 {owned.favor}{owned.star !== undefined ? ` / ${owned.star * 15}` : ''}</span> : null}<span>副本 {owned.spareCopies}</span>{owned.qualityExperience !== undefined ? <span>品质经验 {owned.qualityExperience}</span> : null}</> : <span>尚未获得 · 获取 Lv{skill.dropLevel ?? skill.requiredLevel}</span>}{skill.kind === 'active' ? <span>冷却 {skill.cooldown} 回合</span> : <span>被动槽生效</span>}</p>
