...
import { demonTowerUpgradeCost, type DemonTowerAction, type DemonTowerCatalog, type DemonTowerLoadout, type DemonTowerProfileView, type DemonTowerSkillDefinition, type DemonTowerWeaponDefinition } from '@stealth-reader/shared';
...
export function DemonTowerInventory({ profile, catalog, disabled, onAction }: { profile: DemonTowerProfileView; catalog: DemonTowerCatalog; disabled: boolean; onAction: (action: DemonTowerAction) => Promise<boolean> }): JSX.Element {
  ...
  const cost = upgrade && currentUpgrade ? demonTowerUpgradeCost(upgrade.type, upgrade.definition.id, currentUpgrade.quality, currentUpgrade.spareCopies, profile.level) : null;
  ...
  return <div className={styles.stack}>
    <TowerPanel title="当前配装" detail={<span className={styles.badge}>{dirty ? '有未保存调整' : '已与服务器同步'}</span>}>
      {profile.battle ? <p className={styles.notice}>战斗中不能调整配装或升阶。可以先查阅图鉴，完成或撤离后再更换。</p> : null}
    ...
    <TowerPanel title="物品档案" detail={<span className={styles.muted}>20 件武器 · 16 种技能</span>}>
      ...
      {category === 'weapons' ? <div className={styles.itemGrid}>{visibleWeapons.map((weapon) => { const owned = profile.weapons.find((item) => item.id === weapon.id); const selected = draft.mainHand === weapon.id || draft.artifact === weapon.id; const locked = profile.level < weapon.requiredLevel; return <article className={styles.item} key={weapon.id} data-selected={selected} data-owned={Boolean(owned)}><div className={styles.itemTop}><TowerWeaponArt weaponId={weapon.id} label={weapon.name} className={styles.itemArt} /><div><h3>{weapon.name}{owned ? ` +${owned.quality}` : ''}</h3><span className={styles.itemMeta}>{weapon.rarity} · {weapon.type} · Lv{weapon.requiredLevel}{owned ? ` · 副本 ${owned.spareCopies}` : ' · 尚未获得'}</span></div></div><p className={styles.itemDescription}>{weapon.description}</p><div className={styles.buttonRow}><button type="button" className={styles.button} disabled={!canEquip || !owned || locked || draft.mainHand === weapon.id} onClick={() => selectWeapon(weapon, 'mainHand')}>{draft.mainHand === weapon.id ? '当前主手' : locked ? `需要 Lv${weapon.requiredLevel}` : '选作主手'}</button>{weapon.type === '法器' ? <button type="button" className={styles.button} disabled={!canEquip || !owned || locked || draft.artifact === weapon.id || draft.mainHand === weapon.id} onClick={() => selectWeapon(weapon, 'artifact')}>{draft.artifact === weapon.id ? '当前辅助' : draft.mainHand === weapon.id ? '主手不可重复' : '装配辅助'}</button> : null}<button type="button" className={styles.button} disabled={!canUpgrade || !owned} onClick={() => setUpgrade({ type: 'weapon', definition: weapon })}>升阶</button><button type="button" className={styles.textButton} onClick={() => setDetail({ type: 'weapon', definition: weapon })}>详情</button></div></article>; })}</div> : <div className={styles.itemGrid}>{visibleSkills.map((skill) => { const owned = profile.skills.find((item) => item.id === skill.id); const selected = (skill.kind === 'active' ? draft.activeSkills : draft.passiveSkills).includes(skill.id); const locked = profile.level < skill.requiredLevel; return <article className={styles.item} key={skill.id} data-selected={selected} data-owned={Boolean(owned)}><div className={styles.itemTop}><TowerIcon name={skill.category === '回复' ? 'heart' : skill.category === '维度' ? 'shield' : 'energy'} className={styles.itemArt} /><div><h3>{skill.name}{owned ? ` +${owned.quality}` : ''}</h3><span className={styles.itemMeta}>{skill.rarity} · {skill.kind === 'active' ? '主动' : '被动'} · Lv{skill.requiredLevel}{owned ? ` · 副本 ${owned.spareCopies}` : ' · 尚未获得'}</span></div></div><p className={styles.itemDescription}>{skill.description}</p><div className={styles.buttonRow}><button type="button" className={styles.button} disabled={!canEquip || !owned || locked} onClick={() => selectSkill(skill)}>{selected ? '从配装卸下' : locked ? `需要 Lv${skill.requiredLevel}` : '加入配装'}</button><button type="button" className={styles.button} disabled={!canUpgrade || !owned} onClick={() => setUpgrade({ type: 'skill', definition: skill })}>升阶</button><button type="button" className={styles.textButton} onClick={() => setDetail({ type: 'skill', definition: skill })}>详情</button></div></article>; })}</div>}
      {(category === 'weapons' ? visibleWeapons : visibleSkills).length === 0 ? <p className={styles.empty}>暂时没有这一类物品。探索后可以继续收集，也可以切换完整图鉴查看效果。</p> : null}
    </TowerPanel>
    {detail ? <TowerModal title={detail.definition.name} onClose={() => setDetail(null)}><span className={styles.badge}>{detail.definition.rarity} · Lv{detail.definition.requiredLevel} · 品质上限 +{detail.definition.qualityCap}</span><p style={{ marginTop: 18 }}>{detail.definition.description}</p>{detail.type === 'weapon' ? <p className={styles.muted}>类型：{detail.definition.type}；主属性：{catalog.attributes[detail.definition.attribute]}。基础加成 {detail.definition.baseBonus}，实际总属性以保存后的人物档案为准。</p> : <p className={styles.muted}>{detail.definition.kind === 'active' ? `主动技能，基础冷却 ${detail.definition.cooldown} 回合。` : '被动技能，装入被动槽位后生效。'}技能描述为基础规则，品质提升与战斗状态均以服务端战报为准。</p>}</TowerModal> : null}
    {upgrade && currentUpgrade && cost ? <TowerModal title={`升阶${upgrade.definition.name}`} onClose={() => setUpgrade(null)} footer={<div className={styles.actionsRight}><button type="button" className={styles.button} disabled={disabled} onClick={() => setUpgrade(null)}>暂不升阶</button><button type="button" className={styles.primary} disabled={!canUpgrade || !cost.available || !materialsEnough} onClick={() => { void onAction({ kind: 'upgrade', payload: { itemType: upgrade.type, itemId: upgrade.definition.id } }).then((success) => { if (success) setUpgrade(null); }); }}>确认升至 +{currentUpgrade.quality + 1}</button></div>}><p>当前 +{currentUpgrade.quality} → +{currentUpgrade.quality + 1}。升阶必定成功，重复副本优先用于升阶，不需要办公币。</p>{cost.reason === 'max_quality' ? <p className={styles.notice}>已达到品质上限，剩余副本会保留。</p> : cost.reason === 'level_required' ? <p className={styles.notice}>下次升阶需要角色 Lv{cost.requiredLevel}，你当前为 Lv{profile.level}。</p> : null}{cost.spareCopies ? <p className={styles.notice}>消耗同名副本 ×1，当前拥有 {currentUpgrade.spareCopies} 份。不额外消耗材料。</p> : <div className={styles.resourceList}>{Object.entries(cost.materials).filter(([, value]) => value > 0).map(([key, value]) => <div className={styles.resource} key={key}><TowerIcon name="material" /><strong>{value}</strong><small>{catalog.materials[key as keyof typeof catalog.materials]} · 持有 {profile.materials[key as keyof typeof profile.materials]}</small></div>)}</div>}{!materialsEnough ? <p className={styles.error}>当前绑定材料不足，先探索收集，再来升阶。</p> : null}</TowerModal> : null}
  </div>;
}
