...
export function DemonTowerPage(): JSX.Element {
  ...
  const disabled = state.busy || state.pending || !overview?.writesEnabled;
  ...
  useEffect(() => { setTab('explore'); setHelp(false); }, [state.ownerId]);
  useEffect(() => {
  ...
  return <section className={styles.page} aria-label="九层妖塔工作台" onFocusCapture={(event) => { if (!pointerFocus.current && event.target instanceof HTMLElement) revealTowerControl(event.target); }}>
    ...
    {catalog?.enabled && overview && profile ? <>
      ...
      <div role="tabpanel" id="tower-panel-profile" aria-labelledby="tower-tab-profile" hidden={tab !== 'profile'}><div className={styles.layout}><CharacterSummary name={state.displayName} profile={profile} catalog={catalog} balance={balance} balanceStale={balanceStale} now={state.now} expanded /><div className={styles.stack}><Attributes key={state.ownerId} profile={profile} catalog={catalog} now={state.now} disabled={disabled} onAction={state.act} /><DemonTowerDaily profile={profile} catalog={catalog} disabled={disabled} onAction={state.act} /><TowerPanel title="成长说明"><p className={styles.muted}>妖塔等级、五维和体力独立于平台账号成长。武器类型决定战斗风格；具体效果可在装备技能页查看。探索与修炼累积经验，配装和被动技能会改变有效属性。</p><button type="button" className={styles.button} onClick={() => changeTab('loadout')}>查看我的配装<TowerIcon name="arrow" /></button></TowerPanel></div></div></div>
