...
export function DemonTowerPage(): JSX.Element {
  ...
  return <section className={styles.page} data-expanded={demonTowerExpansionUIEnabled} data-arena-skin={demonTowerExpansionUIEnabled && profile?.expansion?.arena?.skinUnlocked} data-skin={demonTowerExpansionUIEnabled ? profile?.expansion?.skin : 'field'} aria-label="九层妖塔工作台" onFocusCapture={(event) => { if (!pointerFocus.current && event.target instanceof HTMLElement) revealTowerControl(event.target); }}>
    ...
    {catalog?.enabled && overview && profile ? <>
      ...
      <div role="tabpanel" id="tower-panel-profile" aria-labelledby="tower-tab-profile" hidden={tab !== 'profile'}><div className={styles.profileLayout}><CharacterSummary name={state.displayName} profile={profile} catalog={catalog} balance={balance} balanceStale={balanceStale} now={state.now} expanded /><div className={styles.stack}><DemonTowerGrowth key={`growth-${state.ownerId}`} profile={profile} disabled={disabled} onAction={state.act} />{tab === 'profile' ? <DemonTowerAppearancePanel key={`appearance-${autoScope}`} profile={profile} name={state.displayName} ownerId={state.ownerId} disabled={disabled} showSkins={demonTowerExpansionUIEnabled} onAction={state.act} /> : null}<Attributes key={state.ownerId} profile={profile} catalog={catalog} now={state.now} disabled={disabled} onAction={state.act} /><div className={styles.dailyShortcut} aria-label="今日日常摘要"><p>今日活跃 <strong>{profile.daily.activity} / {profile.daily.activityTarget}</strong><small>{profile.daily.rewardClaimed ? '活跃奖励已结算' : profile.daily.activity >= profile.daily.activityTarget ? '活跃目标已达成，前往探索页核验领取' : '继续探索或修炼积累活跃'}</small></p><button type="button" className={styles.button} onClick={() => changeTab('explore')}>查看日常与领取</button></div><TowerPanel title="成长说明"><p className={styles.muted}>妖塔等级、五维和体力独立于平台账号成长。武器类型决定战斗风格；具体效果可在装备技能页查看。探索与修炼累积经验，配装和被动技能会改变有效属性。</p><button type="button" className={styles.button} onClick={() => changeTab('loadout')}>查看我的配装<TowerIcon name="arrow" /></button></TowerPanel></div></div></div>
      ...
      <div role="tabpanel" id="tower-panel-loadout" aria-labelledby="tower-tab-loadout" hidden={tab !== 'loadout'}><DemonTowerInventory key={state.ownerId} profile={profile} catalog={catalog} disabled={disabled} onAction={state.act} onWorkshop={demonTowerExpansionUIEnabled ? () => changeTab('expansion') : undefined} /></div>
      {demonTowerExpansionUIEnabled ? <div role="tabpanel" id="tower-panel-expansion" aria-labelledby="tower-tab-expansion" hidden={tab !== 'expansion'}><DemonTowerExpansion key={`expansion-${autoScope}`} profile={profile} world={overview.world} disabled={disabled} onAction={state.act} onContinueBattle={() => changeTab('explore')} onSupplies={openSupplies} onLoadout={() => changeTab('loadout')} /></div> : null}
