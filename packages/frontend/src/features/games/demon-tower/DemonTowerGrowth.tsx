...
export function DemonTowerLootGuide({ profile }: { profile: DemonTowerProfileView }): JSX.Element {
  ...
  return <TowerPanel title="免费收集与培养" detail={<span className={styles.badge}>等级资格由服务器判定</span>}>
    ...
    <p className={styles.muted}>{profile.expansion ? '免费扩展已启用：重复物品自动转品质经验，达到境界自动兑现；旧副本完整转存，已有更高强化不回退。残魂秘市需要逐次确认，不自动购买。熟练度升星必成，免费升星符概率失败也不降星。' : '升星免费、不会降星；+N仍可用副本或探索材料必定成功升阶，不会自动替你消费副本或购买物品。'}</p>
