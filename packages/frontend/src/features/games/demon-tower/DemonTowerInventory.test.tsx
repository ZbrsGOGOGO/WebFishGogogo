...
describe('compact demon tower inventory and dedicated cultivation navigation',()=>{
  it('bounds every page to six cards and makes every weapon and skill reachable without a request',()=>{
    ...
    click('技能');expect(screen.getByLabelText('物品分页进度')).toHaveTextContent('第 1 / 5 页');const skills:string[]=[];
    for(let page=0;page<5;page++){skills.push(...screen.getAllByRole('article').map(item=>item.getAttribute('aria-label')!));if(page<4)click('下一页');}
    ...
    expect(skills.slice(-2)).toEqual(['师傅驾到', '义结金兰']);
  });
