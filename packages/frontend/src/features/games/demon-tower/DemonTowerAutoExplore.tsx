...
export const AUTO_STOP_REASONS: Record<DemonTowerAutoStopReason, string> = {
  ...
  battle_timeout: '战斗达到回合上限', stamina_empty: '体力不足以继续出发', vip_expired: '期权持有者 权益已到期', session_ended: '原登录会话已结束',
...
function AutoWorkspace({ overview, catalog, now, ownerId, manualPending, onOverview, onPending }: Props): JSX.Element {
  ...
  return <TowerPanel title="委托探索" detail={<span className={styles.badge}>{running ? '服务器执行中' : '期权持有者 便利权益'}</span>}>
