...
import { PaperArenaConnection,type PaperArenaConnectionStatus } from './PaperArenaConnection';
...
const STATUS:Record<PaperArenaConnectionStatus,string>={connecting:'连接中',online:'实时连接',reconnecting:'正在重连',offline:'连接已断开'};
...
export function PaperArenaPage():JSX.Element{
  ...
  return <main className={styles.page}><header className={styles.pageHeader}><div><span>COLLABORATION NOTES / 红蓝演练</span><h1>纸上突围 · 联机协作</h1><p>4–8 人红蓝对战，空位 AI 补齐。服务器判定命中，击败后随机复活。</p></div><Link to="/games/ballpoint-breach">返回单机</Link></header>{error?<p role="alert" className={styles.error}>{error}</p>:null}{!enabled?<p role="status">联机功能暂未开放。</p>:null}{currentRoom?<p className={styles.resume}>你已有房间 <Link to={`/games/ballpoint-breach/arena/${encodeURIComponent(currentRoom)}`}>返回当前对局</Link></p>:null}
    <div className={styles.lobby}><section className={styles.card}><h2>新建一份演练</h2><form onSubmit={event=>{event.preventDefault();void enter(true);}}><label>房间名称<input maxLength={40} minLength={2} required value={name} onChange={event=>{setName(event.target.value);request.current=null;}}/></label><div className={styles.formRow}><label>总人数（含 AI）<select value={capacity} onChange={event=>{setCapacity(Number(event.target.value));request.current=null;}}>{[4,5,6,7,8].map(value=><option key={value} value={value}>{value} 人</option>)}</select></label><label>获胜击败数<input type="number" min={20} max={100} step={1} required value={target} onChange={event=>{setTarget(Number(event.target.value));request.current=null;}}/></label></div><label>房间密码（可留空）<input type="password" minLength={password?4:undefined} maxLength={32} value={password} autoComplete="new-password" onChange={event=>{setPassword(event.target.value);request.current=null;}}/></label><p>不使用邀请码。目标可设 20–100，人数不足由 AI 接替空位；开局前可换队。</p><button type="submit" disabled={busy||!enabled||Boolean(currentRoom)}>{busy?'提交中…':'创建房间'}</button></form></section>
    ...
    <aside className={styles.rules}><h2>短会话说明</h2><p>单局最长 15 分钟；暂不发放办公币，也不改变已有余额和排行榜。房间为临时对局，服务重启可能中断；空房 30 分钟过期。短时掉线由 AI 接替，90 秒后释放席位，回来不会借机恢复满血。</p><p>联机小窗缩小、遮盖或切出时停止你的输入，但比赛不会暂停。原单机仍保留自动暂停体验。</p></aside>
  </main>;
}
...
export function PaperArenaRoomPage():JSX.Element{
  ...
  useEffect(()=>{
    alive.current=true;const controller=new AbortController(),generation=getCommunitySessionGeneration();
    void paperArenaApi.room(roomId,controller.signal).then(next=>{if(alive.current&&generation===getCommunitySessionGeneration())setRoom(next);}).catch(err=>{if(!controller.signal.aborted)setError(errorMessage(err));});
    const socket=new PaperArenaConnection(roomId,{snapshot:next=>{if(alive.current)setRoom(next);},status:next=>{if(alive.current)setStatus(next);},error:next=>{if(alive.current)setError(next);}});connection.current=socket;void socket.connect();
    return()=>{alive.current=false;controller.abort();socket.stop();connection.current=null;};
  },[roomId]);
  async function action(operation:()=>Promise<PaperArenaRoomView|unknown>,leave=false):Promise<void>{
    ...
    try{const next=await operation();if(alive.current&&generation===getCommunitySessionGeneration()){if(leave){connection.current?.stop();navigate('/games/ballpoint-breach/arena');}else if(next&&typeof next==='object'&&'players'in next)setRoom(next as PaperArenaRoomView);setError('');}}catch(err){if(alive.current)setError(errorMessage(err));}finally{locked.current=false;if(alive.current)setBusy(false);}
  }
