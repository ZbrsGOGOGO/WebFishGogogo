  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      if (request && typeof request.then === 'function') {
        void request.then(() => {
          if (signal?.aborted && pointerDocument.pointerLockElement === canvas) {
            pointerDocument.exitPointerLock?.();
            return;
          }
...
export class InputManager {
  ...
  private pointerRequest: AbortController | null = null;
  ...
  requestPointerLock(): Promise<boolean> {
    this.pointerRequest?.abort();
    this.pointerRequest = new AbortController();
    this.canvas.focus({ preventScroll: true });
    return attemptPointerLock(this.canvas, document, 350, this.pointerRequest.signal);
  }
  ...
  get controlsActive(): boolean {
    return this.enabled && (this.pointerLocked || (this.pointerFallback && document.activeElement === this.canvas));
  }
  ...
  private readonly onKeyDown = (event: KeyboardEvent): void => {
    if (!this.controlsActive || document.activeElement !== this.canvas || event.ctrlKey || event.metaKey || event.altKey) return;
    if (!(event.target instanceof Element) || event.target !== this.canvas) return;
    if (!/^(Key[WASDRQ]|ShiftLeft|ShiftRight|Space|Escape|Digit[1-5])$/.test(event.code)) return;
    ...
    if (/^Digit[1-5]$/.test(event.code)) this.weaponSelection = Number(event.code.slice(-1));
  };
  ...
  private readonly onMouseDown = (event: MouseEvent): void => {
    if (!this.controlsActive || (!this.pointerLocked && event.target !== this.canvas)) return;
    if (event.button === 0) {
      this.primary = true;
  ...
  private readonly onMouseMove = (event: MouseEvent): void => {
    if (!this.controlsActive || (!this.pointerLocked && event.target !== this.canvas)) return;
    this.lookX += Number.isFinite(event.movementX) ? event.movementX : 0;
    this.lookY += Number.isFinite(event.movementY) ? event.movementY : 0;
  };
  ...
  private readonly onContextMenu = (event: MouseEvent): void => {
    if (this.controlsActive && (this.pointerLocked || event.target === this.canvas)) event.preventDefault();
  };
