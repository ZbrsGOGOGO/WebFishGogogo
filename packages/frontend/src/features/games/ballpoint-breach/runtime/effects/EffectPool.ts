...
export class EffectPool {
  ...
  /** WebFish: include textures/geometries not currently selected by a pooled mesh. */
  disposeOwnedResources(): void {
    this.clear();
    for (const textures of Object.values(this.splatTextures)) {
      for (const texture of textures) texture.dispose();
    }
    for (const geometry of Object.values(this.goreGeometries)) geometry.dispose();
  }
}
