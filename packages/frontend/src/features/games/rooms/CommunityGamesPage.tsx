...
export function CommunityGamesPage(): JSX.Element {
  ...
  if (COMMUNITY_FEATURE_FLAGS.towerDefense) {
    ...
    add({
      id: 'word-front', title: '文字战线 · 赵云救阿斗', subtitle: filter === 'rooms' ? '双人房间 · V4 双线对攻' : '十二武将 · V4 十行战场', cover: 'word', categories: ['solo', ...(COMMUNITY_FEATURE_FLAGS.wordFrontRooms ? ['rooms' as const] : [])],
      time: filter === 'rooms' ? '对战最多 20 分钟' : '20 波 / 40 波无尽', device: '键鼠 / 触屏', access: '需要登录', save: filter === 'rooms' ? '限时服务端快照' : '本机草稿', ranking: filter === 'rooms' ? '不计正式榜、成就或办公币' : 'V4 独立榜 · 不发办公币',
      description: filter === 'rooms' ? '两位真人红蓝双线对攻；击败来客，向对方投递援军。' : '招募字卡、合成队员，经营阵容守住核心；旧版入口全部保留。',
      rules: <>{filter !== 'rooms' ? <p>剧情与无尽独立于原工位塔防，不扣账号办公币。在线参榜需当前页面完成；刷新或切页后可续本机草稿，但失去当前局参榜资格。</p> : null}{COMMUNITY_FEATURE_FLAGS.wordFrontRooms ? <p>双人房间与单机共用 V4 十行地图、十二武将和局内商人；不计单机榜、办公币或成就。收起画面后服务端仍持续推进，对战最多 20 分钟，有时限的数据库快照可跨 API 重启恢复。</p> : null}<div className={styles.ruleLinks}><Link to="/tower-defense/word-front/v3">旧版护送（V3）</Link><Link to="/tower-defense/word-front/v2">旧版六章（V2）</Link><Link to="/tower-defense/word-front/legacy">初版草稿（V1）</Link><Link to="/tower-defense">原工位塔防</Link></div></>,
