...
export function CommunityGamesPage(): JSX.Element {
  ...
  return <section aria-label="小游戏专区">
    ...
    {COMMUNITY_FEATURE_FLAGS.towerDefense && <section className={styles.panel} aria-label="文字战线独立玩法"><div className={styles.panelHeading}><h2>文字战线 · 赵云救阿斗</h2><span className={styles.muted}>双字组队 · 路径防守</span></div><div className={styles.panelBody}><p>五张字卡招募、两位同阶成员合一，守住五点核心生命。剧情与无尽独立于原工位塔防，不扣账号办公币。</p><div className={styles.rowActions}><Link className={styles.quietButton} to="/tower-defense/word-front">进入文字战线</Link>{COMMUNITY_FEATURE_FLAGS.wordFrontRooms ? <Link className={styles.textLink} to="/tower-defense/word-front/rooms">双人房间（V2 规则，不计正式榜）</Link> : null}<Link className={styles.textLink} to="/tower-defense">原工位塔防</Link></div></div></section>}
