...
export function CommunityLeaderboardsPage(): JSX.Element {
  ...
  return <section className={styles.siteLeaderboardPage} aria-label="排行榜专区">
    ...
    {tab === 'coins' ? <div role="tabpanel" aria-label="办公币余额榜">
      ...
      {COMMUNITY_FEATURE_FLAGS.workstationCampaign && COMMUNITY_FEATURE_FLAGS.towerDefense && <section className={styles.panel}><div className={styles.panelHeading}><h2>工位塔防 · 正式日榜</h2><Link className={styles.quietButton} to="/tower-defense/leaderboard">查看塔防日榜 →</Link></div><div className={styles.panelBody}><p>剧情、无尽、极限独立排名，按服务端结算的个人当日最好成绩排列。各模式有效日榜冠军 12 办公币；本地练习不参榜。</p></div></section>}
      {COMMUNITY_FEATURE_FLAGS.officeHub && <section className={styles.panel}><div className={styles.panelHeading}><h2>公司协作 · 周进度</h2><Link className={styles.quietButton} to="/office">查看公司周榜与协作 →</Link></div><div className={styles.panelBody}><p>公司周目标与个人参与条件均由服务端确认；与个人小游戏日榜分开结算，避免重复奖励。</p></div></section>}
