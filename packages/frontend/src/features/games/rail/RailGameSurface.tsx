...
export function RailGameSurface({ view, onAction, disabled = false }: { view: RailGameView; onAction: (input: RailAction) => Promise<boolean>; disabled?: boolean }): JSX.Element {
  ...
  const isBot = (id: string): boolean => view.players.some((player) => player.id === id && player.isBot);
  const automaticLabel = (id: string): string => isBot(id) ? ' · 机器人自动操作' : ' · 超时托管';
  ...
  return <div className={styles.stack} aria-label="轨道难题赛局">
    ...
    <div id="rail-tracks" className={styles.tracks} aria-label="两条轨道">
      {(['A', 'B'] as RailTrack[]).map((track) => <section className={styles.track} data-chosen={view.chosenTrack === track} key={track} aria-label={`${track} 轨`}>
        ...
        {view.tracks[track].length ? view.tracks[track].map((character) => <article className={styles.character} key={character.id}>
          <small>{character.card.kind === 'good' ? '善牌' : '恶牌'} · {name(character.ownerId)}{character.automatic ? automaticLabel(character.ownerId) : ''}</small><strong>{character.card.title}</strong><p>{character.card.description}</p>
          {character.buff ? <p className={styles.buff}><strong>条件：{character.buff.card.title}</strong>{character.buff.card.description}<small style={{ display: 'block', marginTop: 5 }}>{name(character.buff.ownerId)}{character.buff.automatic ? automaticLabel(character.buff.ownerId) : ''}</small></p> : null}
    ...
    {(view.phase === 'round_end' || view.phase === 'finished') && view.roundResult ? <section id="rail-actions" className={styles.panel}><div className={styles.panelTitle}><h2>第 {view.roundResult.round} 回合记录</h2></div><div className={styles.body}><p>列车经过 {view.roundResult.chosenTrack} 轨{view.roundResult.automaticDecision ? isBot(view.roundResult.conductorId) ? '（机器人自动选择）' : '（超时自动选择）' : ''}；{view.roundResult.survivedPlayerIds.map(name).join('、') || '无人'}生存。</p><p className={styles.muted}>本轮恶魔评分合计 {view.roundResult.demonScore}。{view.roundResult.ratings.some((item) => item.automatic && item.value === null && !isBot(item.playerId)) ? '超时未评分按弃评处理，不替玩家打分。' : ''}</p>{available.includes('next_round') ? <button type="button" className={styles.primary} disabled={!can('next_round')} onClick={() => { void submit({ kind: 'next_round', payload: { roundToken: view.roundToken } }); }}>进入下一回合</button> : view.phase !== 'finished' ? <span className={styles.muted}>等待进入下一回合。</span> : null}</div></section> : null}
