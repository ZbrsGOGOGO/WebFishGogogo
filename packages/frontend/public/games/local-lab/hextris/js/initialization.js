...
function initialize(a) {
	...
	window.op = 0;
	window.saveState = hextrisRead("saveState") || "{}";
	if (saveState !== "{}") {
		op = 1;
	}
	...
	highscores = [];
	if (hextrisRead('highscores')) {
		try {
			highscores = JSON.parse(hextrisRead('highscores'));
		} catch (e) {
			highscores = [];
		}
	}
	...
	if (a != 1) {
		...
		$(window).unload(function() {
			...
			if (gameState == 1 || gameState == -1 || gameState === 0) hextrisWrite("saveState", exportSaveState());
			else hextrisWrite("saveState", "{}");
		});
