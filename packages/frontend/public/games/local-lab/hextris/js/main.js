...
function init(b) {
	...
	$("#pauseBtn").attr('src',"./images/btn_pause.svg");
	hideUIElements();
	var saveState = hextrisRead("saveState") || "{}";
...
function showHelp() {
	...
	$("#inst_main_body").html("<div id='instructions_head'>玩法说明</div><p>不要让方块堆出外围灰色六边形。</p><p>" + (settings.platform != 'mobile' ? '用左右方向键或 A / D' : '点击屏幕左边或右边') + "旋转中心六边形；3 个以上同色相连即可消除。方向下键可加速落下。</p><p>外围彩色线表示连续消除奖励的剩余时间。P / 空格暂停，点击画面继续。</p><hr><p>Hextris 原作：Logan Engstrom、Garrett Finucane、Noah Moroze、Michael Yang。本站 2026-09-15 自托管静音改编，GPL-3.0-or-later；无担保。成绩只保留在当前游戏内存，关闭后不保存。</p>");
	if (gameState == 1) {
		pause();
	}
