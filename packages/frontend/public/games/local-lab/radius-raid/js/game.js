...
$.init = function () {
  ...
  $.mute = true; // This self-hosted edition is always silent.
...
$.resizecb = function (e) {
  // Reserve the unscaled accessible toolbar without covering the combat HUD.
  let viewport = document.getElementById("stage");
  let winWidth = Math.max(1, viewport ? viewport.clientWidth : window.innerWidth);
  let winHeight = Math.max(1, viewport ? viewport.clientHeight : window.innerHeight);
...
$.setState = function (state) {
  ...
  if (state == "stats") {
    ...
    let clearButton = new $.Button({
      ...
      action: function () {
        ...
        if (
          window.confirm(
            "Reset the statistics for this open game window? Closing the window also clears them."
          )
        ) {
          $.clearStorage();
          $.mouse.down = 0;
  ...
  if (state == "credits") {
    ...
    let js13kButton = new $.Button({
      ...
      title: "MENU",
      action: function () {
        $.setState("menu"); // Credits do not open another site.
        $.mouse.down = 0;
      },
    });
  ...
  // set state
  $.state = state;
  if ($.syncAccessibleControls) $.syncAccessibleControls();
};
...
$.loop = function () {
  ...
  // Audio is intentionally absent from the self-hosted edition.
