...
import { createTileActivation, getTileA11yLabel } from "./tileInteraction"
...
export function TileComponent(iProps: Props) {
  ...
  const canActivate = createMemo(() => canBeSelected() && !props.tile.deleted && !animation() && !game.pause && !game.endCondition)
  const interaction = createTileActivation(() => !canActivate(), () => {
    play("click")
    ...
    <>
      ...
      >
        <div
          ...
          role="button"
          aria-label={getTileA11yLabel(props.tile.id, card(), canBeSelected() && !props.tile.deleted)}
          aria-disabled={!canActivate()}
          tabindex={canActivate() ? 0 : -1}
          onPointerDown={interaction.pointer}
          onKeyDown={interaction.keyDown}
          onKeyUp={interaction.keyUp}
