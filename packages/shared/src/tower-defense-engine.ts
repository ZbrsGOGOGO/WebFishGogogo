...
export interface TowerDefenseState {
  campaign?: {
    job: 'specialist' | 'admin' | 'it' | 'hr'; talents: { output: number; control: number; economy: number };
    chapter: number; mode: 'story' | 'endless' | 'extreme'; slots: number; totalWaves: number;
    streak: number; bestStreak: number; waveBreaches: number; safeTicks: number;
    plantHurtTick: number; bag: { x: number; y: number; expires: number; coins: number } | null;
    plantHp?: number; mergedThree?: boolean;
    boostUntil: number; weekend: 0 | 6 | 7; event: string; eventUntil: number;
    collectedBags: number; countdown: number; completedWaves: number;
    skill2Until?: number; skill3Until?: number; waveStartedTick?: number; fastestWaveTicks?: number;
  };
...
function randomOffer(
  ...
  const rare = nextRandom(random.seed);
  return {
    seed: rare.seed,
    offer: {
      id: `offer-${offerId}`,
      ...
      source: 'random',
      rarity: rare.value < .01 ? 'SSR' : rare.value < .05 ? 'SR' : 'R',
    },
  };
}
...
export function moveTowerDefenseHero(
  ...
  const bag = next.campaign?.bag;
  if (bag && distance(next.hero, bag) <= 1) return { ...next, credits: next.credits + bag.coins,
    campaign: { ...next.campaign!, bag: null, boostUntil: next.tick + 11, collectedBags: next.campaign!.collectedBags + 1, event: `收取加班福袋 +${bag.coins} G`, eventUntil: next.tick + 11 } };
  return next;
}
...
export function upgradeTowerDefensePlant(state: TowerDefenseState): TowerDefenseActionResult {
  ...
  return makeFeedback(
    { ...state, credits: state.credits - cost, plantLevel: level, ...(state.campaign?{campaign:{...state.campaign,plantHp:6+level*3}}:{}) },
    true,
    'ok',
  ...
  return makeFeedback(
    {
      ...
      ...(state.campaign && didMerge && merged.finalItem.tier === 3 ? {campaign:{...state.campaign,mergedThree:true}} : {}),
      shop,
  ...
  return makeFeedback(
    {
      ...
      ...(state.campaign && level === 3 ? {campaign:{...state.campaign,mergedThree:true}} : {}),
...
function collectDefeated(state: TowerDefenseState): TowerDefenseState {
  ...
  return {
    ...
    credits: state.credits + defeated.reduce((sum, enemy) => sum + enemy.reward, 0) * ((state.campaign?.streak ?? 0) >= 5 ? 2 : 1),
...
export function triggerFocusPulse(state: TowerDefenseState): TowerDefenseState {
  ...
  const range = state.hero.range + 1 + (state.campaign?.talents.control ?? 0);
  ...
  return collectDefeated({
    ...state,
    hero: { ...state.hero, hp: state.campaign?.job === 'admin' ? Math.min(12, (state.hero.hp ?? 12) + 3) : state.hero.hp, pulseCooldown: Math.max(10, 20 - (state.campaign?.talents.control ?? 0) * 2), lastPulseTick: state.tick },
    effects: [makeEffect(
      state.tick,
      0,
      'pulse',
    ...
    enemies: state.enemies.map((enemy) =>
      targetIds.has(enemy.id) ? (() => {
        const hit = damageEnemy(enemy, state.hero.attack * (state.hero.level >= 3 ? 3 : 2), { area: true, trueDamage: state.campaign?.job === 'it' });
        return state.campaign?.job === 'hr' ? controlEnemy(hit, 'stunTicks', 4) : hit;
      })() : enemy,
    ),
  });
}
...
function runAttacks(state: TowerDefenseState): TowerDefenseState {
  ...
  if (hero.autoCooldown === 0 && (hero.hp ?? 1) > 0) {
    ...
    if (target) {
      ...
      enemies[index] = damageEnemy(target, hero.attack, { trueDamage: state.campaign?.job === 'it' });
      if (state.campaign?.job === 'hr') enemies[index] = { ...enemies[index], slowTicks: 3 };
      hero = { ...hero, autoCooldown: Math.max(2, 4 - Math.floor(hero.level / 2)) };
    }
  }

  const synergyCounts = Object.fromEntries(TOWER_TYPES.map((type) => [type, state.campaign ? state.towers.filter((tower) => tower.type === type).length : 0]));
  const towers = state.towers.map((tower) => {
    const count = synergyCounts[tower.type] ?? 0;
    const adjacent = tower.type === 'single' && count >= 2 && state.towers.some((other) => other.id !== tower.id && other.type === tower.type && distance(TOWER_SLOTS[other.slotIndex]!, TOWER_SLOTS[tower.slotIndex]!) <= 3);
    const nextTower = { ...tower, cooldown: Math.max(0, tower.cooldown - (adjacent && state.tick % 5 === 0 ? 2 : 1)) };
    ...
    if (tower.type === 'single') {
      ...
      return { ...nextTower, cooldown: count >= 3 && tower.level === 3 ? 1 : tower.level === 3 ? 2 : 3 };
    }
    ...
    if (tower.type === 'push') {
      ...
      for (const candidate of affected) {
        ...
        enemies[candidate] = controlEnemy({ ...damaged, pathIndex }, 'stunTicks', (tower.level === 3 ? 2 : 1) + (count >= 2 ? 4 : 0));
      }
    ...
    if (tower.type === 'shred') {
      ...
      for (const candidate of affected) {
        const enemy = enemies[candidate];
        const damaged = damageEnemy(enemy, (tower.level === 3 ? 4 : 2) * (count >= 2 && enemy.archetype === 'basic' ? 1.5 : 1), { area: tower.level === 3, trueDamage: enemy.archetype === 'basic' });
    ...
    for (let enemyIndex = 0; enemyIndex < enemies.length; enemyIndex += 1) {
      ...
      const inArea = tower.type === 'splash' && tower.level === 2
        ? sameLaserLine && distance(point, center) <= (count >= 2 ? 3 : 2)
        : Math.abs(enemy.pathIndex - targetPathIndex) <= (tower.type === 'splash' && count >= 2 ? 2 : 1);
    ...
    if (tower.type === 'slow') {
      for (const enemyIndex of affectedIndexes) {
        ...
        enemies[enemyIndex] = {
          ...damaged,
          slowTicks: count >= 2 ? Math.min(30, damaged.slowTicks + 7) : Math.max(damaged.slowTicks, tower.level === 3 ? 9 : 7),
        };
      }
  ...
  if (state.campaign) {
    const multiplier = 1 + state.campaign.talents.output * .04 + (state.campaign.streak >= 5 ? .25 : state.campaign.streak >= 3 ? .10 : 0) + (state.campaign.boostUntil > state.tick ? .20 : 0) + (state.campaign.job === 'admin' ? .08 : 0);
    enemies = enemies.map((enemy) => { const before = state.enemies.find((entry) => entry.id === enemy.id); return before && before.hp > enemy.hp ? { ...enemy, hp: enemy.hp - Math.floor((before.hp - enemy.hp) * (multiplier - 1)) } : enemy; });
    if (state.tick % 36 === 0 && (synergyCounts.slow ?? 0) >= 3) enemies = enemies.map((enemy) => controlEnemy(enemy, 'freezeTicks', 4));
    if (state.tick % 36 === 18 && (synergyCounts.push ?? 0) >= 3) enemies = enemies.map((enemy) => controlEnemy(enemy, 'stunTicks', 2));
  }
...
function moveEnemies(state: TowerDefenseState, tick: number): TowerDefenseState {
  ...
  if (campaign) {
    const attackers = enemies.filter((enemy) => distance(enemyPoint(enemy), hero) <= 1);
    if (attackers.length && tick % 8 === 0 && (hero.hp ?? 12) > 0) hero = { ...hero, hp: Math.max(0, (hero.hp ?? 12) - attackers.length), hurtTick: tick, hurtAmount: Math.min(hero.hp??12,attackers.length) };
    if (!attackers.length && tick % 18 === 0) hero = { ...hero, hp: Math.min(12, (hero.hp ?? 12) + 1) };
    // A guard near the exit absorbs a real point of breach damage at a real HP cost.
    if (coreDamage > 0 && (hero.hp ?? 12) >= 2 && distance(hero, TOWER_DEFENSE_PATH[TOWER_DEFENSE_PATH.length - 1]!) <= 2) {
      coreDamage -= 1; hero = { ...hero, hp: (hero.hp ?? 12) - 2, hurtTick: tick, hurtAmount:(hero.hurtTick===tick?(hero.hurtAmount??0):0)+2 };
    }
    campaign = { ...campaign, safeTicks: coreDamage > 0 ? 0 : campaign.safeTicks + 1, plantHurtTick: coreDamage > 0 ? tick : campaign.plantHurtTick, plantHp:Math.max(0,(campaign.plantHp??(state.plantLevel?6+state.plantLevel*3:0))-coreDamage) };
  }
  return { ...state, hero, campaign, enemies, breached, coreHp: Math.max(0, state.coreHp - coreDamage) };
}

export function advanceTowerPlantIncome(state: TowerDefenseState): TowerDefenseState {
  if (state.plantLevel <= 0) return { ...state, plantIncomeTick: 0 };
  if (state.campaign && (state.campaign.plantHp??1)<=0) return {...state,plantIncomeTick:0};
  ...
  return {
    ...state,
    plantIncomeTick: 0,
    credits: state.credits + Math.floor(plantIncomePerPayout(state.plantLevel) * (1 + (state.campaign?.talents.economy ?? 0) * .08) * (state.campaign && state.campaign.safeTicks % 54 >= 36 ? 1.5 : 1)),
  };
}
...
export function stepTowerDefense(state: TowerDefenseState): TowerDefenseState {
  ...
  let next = advanceTowerPlantIncome({ ...state, tick, effects: [] });
  ...
  if (next.spawnQueue.length === 0 && next.enemies.length === 0) {
    ...
    if (next.campaign) {
      const perfect = next.breached === next.campaign.waveBreaches;
      const streak = perfect ? next.campaign.streak + 1 : 0;
      next = { ...next, campaign: { ...next.campaign, completedWaves: next.campaign.completedWaves + 1, streak, bestStreak: Math.max(streak, next.campaign.bestStreak), waveBreaches: next.breached, countdown: 18, fastestWaveTicks: Math.min(next.campaign.fastestWaveTicks ?? Number.MAX_SAFE_INTEGER, tick-(next.campaign.waveStartedTick ?? 0)) } };
      if (next.campaign!.mode === 'extreme' && next.wave === 1 && (!perfect || !next.towers.some((tower) => tower.level === 3))) return { ...next, status: 'lost', lastAction: { ok: false, code: 'invalid_status', message: '极限挑战：首轮须零漏怪且部署三星主力，才有下班资格。', tick } };
    }
    return next.wave >= (next.campaign?.totalWaves ?? TOWER_DEFENSE_WAVES)
...
export function startNextTowerDefenseWave(state: TowerDefenseState): TowerDefenseState {
  if (state.status !== 'intermission' || state.wave >= (state.campaign?.totalWaves ?? TOWER_DEFENSE_WAVES)) return state;
  const wave = state.wave + 1;
  return {
    ...state,
    status: 'running',
    wave,
    spawnQueue: state.campaign ? campaignSpawns(wave, state.campaign.chapter, state.campaign.mode) : waveSpawns(wave),
    nextSpawnAt: state.tick + 1,
    effects: [],
  };
}

export function campaignSpawns(wave: number, chapter: number, mode: 'story' | 'endless' | 'extreme'): TowerRoundEnemySpawn[] {
  const second = wave % 2 === 0;
  const base = waveSpawns(second ? 2 : 1);
  const scale = 1 + (chapter - 1) * .24 + Math.floor((wave - 1) / 2) * .18 + (mode === 'extreme' ? .25 : 0);
  return base.map((spawn) => ({ ...spawn, hp: Math.ceil(spawn.hp * scale), score: Math.ceil(spawn.score * scale), name: wave > 2 ? `${spawn.name}·${Math.ceil(wave / 2)}批` : spawn.name }));
}
