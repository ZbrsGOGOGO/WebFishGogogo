...
本文记录 2026-09-15 六款原版游戏的自托管接入、源码和使用边界，不是全站验收记录。已上线的实际版本、验收和回滚依据见[第三阶段发布记录](RELEASE_GAME_PACK_20260915.md)；不表示其他游戏、协作反馈或玩家建房已全部完成。
...
[Web 配置](../deploy/community.nginx.conf) 本批只新增这六个已审阅实验室游戏文档的本站嵌入例外；既有遮司例外及站点外壳原有禁止嵌入规则保留。六款游戏文档 `connect-src 'none'`，静态脚本不需要 `unsafe-inline` 或 `unsafe-eval`；允许内联样式不等于允许内联脚本。缺失资源返回 404，不回退成 SPA HTML 冒充脚本。少数保留通用库有未用的网络方法，游戏不调用，连接 CSP 仍禁止它们联网。
