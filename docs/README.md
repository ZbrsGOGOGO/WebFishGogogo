...
最新发布：[妖塔出发回执与异步猜画反馈修复](RELEASE_COLLABORATION_20260915.md)：最近探索出发结果可刷新恢复，猜画免费不限每日创作次数、支持 1–5 分评价，公司深色纸张对比修复；本次更新 API/Web，无迁移或新游戏。此前[六款本地小游戏第三阶段](RELEASE_GAME_PACK_20260915.md)的 27 个目录入口与低调工作稿、[个人工作台与成长档案第二阶段](RELEASE_UI_PHASE2_20260915.md)、[第一阶段首页与游戏大厅](RELEASE_UI_PHASE1_20260915.md)、原[文字战线 v3 与免费双人房](RELEASE_WORD_FRONT_V3_20260914.md)、[协作反馈修复](RELEASE_FEEDBACK_20260914.md)、[压力整理与统一导航](RELEASE_RELIEF_NAVIGATION_20260911.md)、[妖塔提案适配](DEMON_TOWER_PROPOSALS_20260911.md)、[灵石经济](DEMON_TOWER_ECONOMY.md)、[成员投稿与站长审核](RELEASE_POSTING_20260910.md)、[深色模式](RELEASE_DARK_MODE_20260910.md)、[工作台与桌宠增强](MY_WORKSPACE.md)能力保留。

更新日期：2026-09-15。先看[项目首页](../README.md)与[当前进度](PROGRESS.md)，再按下面的主题查阅。

2026-09-15 **18:06:56（北京时间）**已完成反馈修复发布：API/Web 均固定到 `f346f0c413d653203ae7f4c8d9ed2eadb8276e76`，API 镜像 `sha256:6f4d25ca2204c47d9581586aa71e7162e21346fe962538b7020c2d18d2af57ee`，Web 镜像 `sha256:04ea075460d0116dbc48e135e525e2afef9c01e0905f485a44a1a32bfa745eb8`。只切换 API/Web 与 `IMAGE_TAG`；PostgreSQL、Redis、Gateway 完整身份及数据卷不变，数据库保持 **schema39 / 40 条迁移 / 148 张 public 表**，不执行 UP/DOWN。GitHub `main` 已正常同步应用代码，后续文档提交不代表重新部署应用。

本批前端 219 文件 / 1934 项通过；完整后端基线 161 套 / 1626 项通过，另 6 套 / 84 项条件跳过，最终后端专项 5 套 / 206 项通过。最终不可变镜像的隔离真实 PostgreSQL 2 套 / 58 项、四模块 Nest HTTP 12 组 / 60 请求通过；后者不是完整 community 根模块或生产账号验收。新鲜备份离站校验、148 表完整隔离恢复与候选/兼容桥 40/40 迁移映射只读比对通过；最终公网 65 项 HTTP 烟测、104 个发行文件哈希通过。**生产认证 Cookie 烟测明确跳过**，不宣称全站认证、真人手机矩阵或长期经济平衡已验收，具体范围见[本次发布](RELEASE_COLLABORATION_20260915.md)。

协作反馈已实际回写并于 **18:13:16（北京时间）**复核：33 条中 31 已完成、2 进行中、0 待审阅。探索回执反馈为已完成 v3、4/4；异步猜画为已完成 v5、3/3。两份大型提案仍为进行中 v5：妖塔 v0.11 6/11、塔防更改 4/9，不能据此宣称全部提案完成。足球及新游戏来源仅只读评估，未接入；Palm AI 与其暂停依赖/接口文件仍未提交、未部署，不新增支付或权限。
...
| 九层妖塔 | [出发回执恢复](RELEASE_COLLABORATION_20260915.md)、[v0.8—v0.11 适配](DEMON_TOWER_PROPOSALS_20260911.md)、[六工作区整合](RELEASE_COLLABORATION_20260912.md)、[免费技能与战报补齐](RELEASE_WORD_FRONT_V3_20260914.md)、[历史显示修复](RELEASE_FEEDBACK_20260914.md)、[灵石经济与集中申领](DEMON_TOWER_ECONOMY.md)、[基础玩家指南](DEMON_TOWER_PLAYER_GUIDE.md)、[免费扩展规则](../packages/backend/src/modules/community/demon-tower/EXPANSION.md) | 六个工作区、申领五分类；最近服务器出发回执可刷新恢复，同步前禁止新出发，不把即时消耗/到账混为整局收益。原 70% 战斗 / 15% 宝匣 / 15% 机缘保留，普通探索即使非战斗仍消耗 5 体力。旧灵石/残魂不扣办公币，新专区确认扣款，无真实付费；v0.11 后续规划未全部完成 |
...
- [摸鱼指数与期权持有者](COMMUNITY_FISH_SUPPORT.md)：有效活跃计时、六级摸鱼头衔、人工核验台账及第二阶段页面组织；schema36 是 2026-09-10 首次增量升级历史，当前 schema39，本批无迁移，VIP 为历史名称。
...
- [本次协作反馈修复](RELEASE_COLLABORATION_20260915.md)：探索回执恢复与异步猜画不限每日次数/评价按各自范围完成并回写；2026-09-15 18:13:16 的快照为 33 条（31 完成、2 进行中、0 待审阅），仍有两份大型提案未完成。
...
当前 schema39、40 条迁移历史、148 张 public 表；本次只更新 API/Web，不执行迁移。**本批直接回滚为 API+Web**：API 使用兼容桥 `11eae10148766db7b64b649cde14bfd31feb5c62` 镜像 `sha256:323679fb4ecc474ba585844e3157d2d84e88c9016fc1846513f17d005c59872d`，Web 恢复已保留的第三阶段 `2b7b8d9c03a99cea0a7e1fd3e66f714e68baceb9` 镜像 `sha256:3799fae5386a3eab46b61aea9e77d7675405c725369de0e50fd6297adebee91a`；使用本批完整旧环境，只将标签改为桥版本。桥仅补注销时的 ratings 清理，裸旧 API `832f528` 不直接回切。保留全部玩家数据和 40 条迁移，不执行 DOWN、旧库回灌、权限/权益重放；PostgreSQL、Redis、Gateway 及数据卷不动，但 API 回切会结束内存临时房间。不要套用第三阶段 Web-only 或更早 API 桥流程，详见[本次发布记录](RELEASE_COLLABORATION_20260915.md)。

## 正式发布记录

同一天的记录按功能发布先后演进，不能只按文件日期判断版本。第一项为当前 API/Web `f346f0c`，其余为历史发布记录。文档提交可以晚于运行应用，不代表再次部署。

| 阶段 | 记录 |
| --- | --- |
| 当前 API/Web：妖塔出发回执、异步猜画与公司对比修复，f346f0c | [2026-09-15 协作反馈修复发布](RELEASE_COLLABORATION_20260915.md) |
