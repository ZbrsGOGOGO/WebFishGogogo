...
截至最近一次已验收发布，线上应用为 `f46a765271a55d5708c6f6e0299c149b4183ef11`，数据库为 schema35。本轮 `main` 整理以“已部署应用代码 + README / 文档”为边界，不把未上线应用改动加入基线，也不表示文档提交已经部署。确认运行版本、发布结果和回滚边界时，以对应的**正式发布记录**为准；设计文档中的“待验收”或旧提交号是该阶段的历史，不是新的部署指令。
...
| 成就、称号与 VIP | [成长系统设计](COMMUNITY_PROGRESSION_20260909.md)、[成长发布](RELEASE_GROWTH_20260909.md)、[补齐发布](RELEASE_COMPLETION_20260909.md) | 初版 12 项成就已扩展为 35 项；VIP 曾向当时 active 账号一次性赠送，不是管理员角色或持续新账号赠送 |
...
最新 Paper 发布未执行迁移，schema35 保持 36 条迁移历史、146 张 public 表。历史发布中的迁移 DOWN、旧库恢复、成员授权及 VIP 赠送不能照抄到生产；生产回退以选定版本对应的正式预案为准。
...
| 阶段 | 记录 |
| --- | --- |
| 当前应用：Paper 原版场景联机 v2，f46 | [2026-09-09 Paper v2 发布](RELEASE_PAPER_V2_20260909.md) |
