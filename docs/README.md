...
最新发布：[免费压力整理与统一工作区导航](RELEASE_RELIEF_NAVIGATION_20260911.md)，新增活跃机会、绑定解压币、真实联动奖励与称号；跨页目录支持当前账号在本浏览器内排序、隐藏和重置，详见[压力整理规则](OFFICE_RELIEF.md)。原有[妖塔提案适配](DEMON_TOWER_PROPOSALS_20260911.md)、[灵石经济](DEMON_TOWER_ECONOMY.md)、[成员投稿与站长审核](RELEASE_POSTING_20260910.md)、[深色模式](RELEASE_DARK_MODE_20260910.md)、[工作台与桌宠增强](MY_WORKSPACE.md)能力保留。

更新日期：2026-09-11。先看[项目首页](../README.md)与[当前进度](PROGRESS.md)，再按下面的主题查阅。

截至 2026-09-11 17:02:37（北京时间）的已验收发布，线上应用为 `a84759f9caae25fb15fa0ffb756af1f8ccd437ee`，数据库为 schema36，GitHub `main` 已快进同步。确认运行版本、发布结果和回滚边界时，以对应的**正式发布记录**为准；设计文档中的“待验收”或旧提交号是该阶段的历史，不是新的部署指令。后续文档提交不代表重新部署应用。
...
- [历史妖塔提案发布与反馈处理](RELEASE_PROPOSALS_20260911.md)：v0.8/v0.9/v0.10 已完成，各为记录 v3、分项 4/4、6/6、5/5；v0.11 仍进行中，记录 v2、4/11，保留 7 个未来待办，不将四份规划全部标为完成。
...
摸鱼指数与支持台账发布新增 0036，当前仍为 37 条迁移历史、148 张 public 表；本次压力整理与导航没有 SQL 迁移。当前回滚目标 `9413977`：先停止新 API/Web 写入，再恢复本批旧环境与旧应用，保留 relief JSON、真实种植经验、全部奖励/回执/称号及其他资产、投稿与审计，**无需关闭妖塔或自动探索**。不恢复旧库覆盖数据；历史 941 → 342 的暂停措施、迁移 DOWN、成员授权及权益赠送不能照抄到本批，具体按当前发布预案执行。
...
| 阶段 | 记录 |
| --- | --- |
| 当前应用：压力整理与统一导航，a84759f | [2026-09-11 17:02 压力整理与导航发布](RELEASE_RELIEF_NAVIGATION_20260911.md) |
