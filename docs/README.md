...
最新发布：[九层妖塔提案适配](RELEASE_PROPOSALS_20260911.md)，八部位形象、配装战力、真实好友、探索统计/符、办公币补给/阶梯箱/碎片与统一养成手册集中到原入口，详见[适配规则](DEMON_TOWER_PROPOSALS_20260911.md)。原有[灵石经济](DEMON_TOWER_ECONOMY.md)、[成员投稿与站长审核](RELEASE_POSTING_20260910.md)、[深色模式](RELEASE_DARK_MODE_20260910.md)、[工作台与桌宠增强](MY_WORKSPACE.md)能力保留。

更新日期：2026-09-11。先看[项目首页](../README.md)与[当前进度](PROGRESS.md)，再按下面的主题查阅。

截至 2026-09-11 14:41:39（北京时间）的已验收发布，线上应用为 `9413977c973e1146a9c70ef02875251e2a2ffbd2`，数据库为 schema36，GitHub `main` 已快进同步。确认运行版本、发布结果和回滚边界时，以对应的**正式发布记录**为准；设计文档中的“待验收”或旧提交号是该阶段的历史，不是新的部署指令。后续文档提交不代表重新部署应用。
...
- [本次提案发布与反馈处理](RELEASE_PROPOSALS_20260911.md)：v0.8/v0.9/v0.10 已完成，各为记录 v3、分项 4/4、6/6、5/5；v0.11 仍进行中，记录 v2、4/11，保留 7 个未来待办。已回写并独立复核，不将四份规划全部标为完成，也不推断全局反馈总量。
...
摸鱼指数与支持台账发布新增 0036，当前仍为 37 条迁移历史、148 张 public 表；本次妖塔提案适配没有 SQL 迁移。回滚 `3426077` 须先停止新 API，再禁用旧版妖塔及自动探索，保留新增 `appearance` / `provisions` JSON 和全部资产、投稿与审计，投稿与审核保持开启，不恢复旧库覆盖数据。历史发布中的迁移 DOWN、成员授权及权益赠送不能照抄到生产，具体按当前发布预案执行。
...
| 阶段 | 记录 |
| --- | --- |
| 当前应用：妖塔提案适配，9413977 | [2026-09-11 14:41 提案发布](RELEASE_PROPOSALS_20260911.md) |
