...
- 用户规模目标：4000；正式公开版当前无需账户，这表示访问用户规划量。以后开放账户时，再用 4000 条合成用户数据验证数据库规模。
- 同时在线用户：1000；当前工具和游戏是纯静态、纯客户端功能，“在线”表示一段时间内持续活跃的访问者，不表示 1000 条长连接。
- 稳态吞吐：400 RPS，等价于 1000 个在线用户平均每 2.5 秒发出一次请求。
- 峰值吞吐：800 RPS，持续 5 分钟；用于覆盖页面切换、集中登录后的读取和活动开始时的短时突发。
- 当前正式公开版默认 100% 测试公开页面和静态资源。以后开放账户与 API 后，传入专用测试令牌会启用默认 70% 公开页面、30% 已登录只读 API 的混合；可用 `AUTH_SHARE` 调整，但一次测试的公开与认证 RPS 之和始终等于该档位的总 RPS。
...
公开请求按权重访问 `/`、`/tools`、`/games`、四个游戏直达页（`/games/snake`、`/games/tetris`、`/games/tank`、`/games/three-sum`）、`/privacy-policy` 和 `/terms-of-service`。认证场景只执行以下 GET 请求：
...
k6 是 HTTP 压测器，不会像浏览器一样解析 SPA 并自动下载懒加载游戏包。只请求 `/games/tetris` 等页面时测到的是 HTML Shell，不代表游戏 JS/CSS/图片冷启动已经被覆盖。
...
该场景验证源站/CDN 的静态吞吐，但不会模拟浏览器并行下载、解析与渲染。游戏真实首屏体验还应使用浏览器性能测试单独验收 LCP、JS 下载/解析时间和缓存命中；1000 用户集中冷启动时，首先关注 CDN 回源率和出口带宽，而不是游戏 API。
...
4. 文档正文从单机卷切到 COS/S3 兼容对象存储，避免多 API 实例共享本地文件产生不一致。
5. Redis 只承担缓存、分布式限流和队列协调；用户资产仍以 PostgreSQL 为唯一真源。
...
扩展到多个 Worker 前，先在真实 PostgreSQL 上完成双事务领取测试，确认
`FOR UPDATE SKIP LOCKED` 能让不同 Worker 领取不同事件；同时检查 Outbox 查询执行计划、待处理数和最老事件年龄，队列变大后再按实测补充索引。
...
- Worker：Outbox 待处理数、最老事件年龄、重试/死信数量。
