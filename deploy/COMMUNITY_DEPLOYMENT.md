...
### 2026-09-11 14:41 当前发布：妖塔提案适配

当前应用 `9413977c973e1146a9c70ef02875251e2a2ffbd2`，北京时间 14:41:39 上线，GitHub `main` 已快进同步。八部位形象、当前配装战力、真实好友切磋、探索统计/符、办公币补给/阶梯箱/技能碎片与养成手册统一到原七个工作区，物资申领现为五分类。旧灵石、残魂渠道仍不扣办公币，新办公币专区明确确认后扣全站钱包，不接真实付费。[功能适配](../docs/DEMON_TOWER_PROPOSALS_20260911.md) · [正式发布与回滚](../docs/RELEASE_PROPOSALS_20260911.md)。
...
直接回滚基线 `34260773f613b540bb9fa4257b8112ccbc87d019`：**先停止新 API，再禁用旧版妖塔整体及自动探索**，保留新增 `appearance` / `provisions` JSON、全部钱包资产/流水、帖子与审计；投稿写入和审核继续开启。只回切 API/Web，不执行迁移 DOWN、旧库覆盖、新字段删除、权限或赠送重放。下列旧发布的回滚开关只适用于其对应历史版本，不能当作本轮步骤。
...
| --- | --- | --- |
| `3426077` | schema36 → 36，无迁移 | [灵石经济与统一物资申领](../docs/RELEASE_ECONOMY_20260911.md)；是本轮 9413977 的应用回滚基线，回退时须保留新增 JSON 并暂停旧版妖塔/自动写入 |
