...
### 2026-09-18 候选迁移复核：文字战线 V4（尚未代表已经上线）

候选应用新增唯一迁移 `1700000000040-AddWordFrontV4ArcadeGames`：在两张既有 Arcade 表的游戏键约束中追加 `word_story_v4` / `word_endless_v4`，新增有时限的 `word_front_room_snapshots` 与独立的 `word_front_map_drafts`，并写入三张内置 10×8 地图草稿。它不改旧成绩、不删除旧房间或玩家资产、不改变办公币；地图草稿与不可变正式排行地图分离。生产升级必须从下方当前 schema39 的新鲜备份恢复副本演练准确的 0039→0040 UP，核对 41/41 迁移、150 张 public 表、约束及旧行指纹后，才可运行生产迁移。

迁移 DOWN 只允许在 V4 成绩为零且地图从未编辑时执行，正常应用回退保留 schema40 和全部 V4 数据，**不执行 DOWN、不回灌旧库**。直接回切旧应用前还必须在恢复副本验证旧妖塔读写能保留未知 `soulBeads` 状态、旧 Arcade 查询不会因 V4 行失败；不通过就先构建 schema40 兼容桥。以下 2026-09-15 段仍是当前已上线事实，候选通过备份、迁移、镜像与公网验收后再以实际提交、镜像和时间替换本段。
