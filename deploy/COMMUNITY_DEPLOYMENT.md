...
### 2026-09-11 17:02 当前发布：压力整理与统一工作区导航

当前应用 `a84759f9caae25fb15fa0ffb756af1f8ccd437ee`，北京时间 17:02:37 上线，GitHub `main` 已快进同步。新增免费活跃机会挑战、绑定解压币、真实种植经验/妖塔物资与三款称号，保留原每日巡视；统一跨页目录支持本账号当前浏览器内排序、隐藏和重置，不跨设备云同步、不改变权限或工作台收藏。[玩法规则](../docs/OFFICE_RELIEF.md) · [正式发布与回滚](../docs/RELEASE_RELIEF_NAVIGATION_20260911.md)。
...
该次直接回滚基线 `34260773f613b540bb9fa4257b8112ccbc87d019`：**先停止新 API，再禁用旧版妖塔整体及自动探索**，保留新增 `appearance` / `provisions` JSON、全部钱包资产/流水、帖子与审计；投稿写入和审核继续开启。只回切 API/Web，不执行迁移 DOWN、旧库覆盖、新字段删除、权限或赠送重放。该暂停要求只适用于对应历史版本，不是当前 a84759f → 9413977 的回滚步骤。
...
| --- | --- | --- |
| `9413977` | schema36 → 36，无迁移 | [妖塔提案适配](../docs/RELEASE_PROPOSALS_20260911.md)；当前 a84759f 的回滚基线，兼容探针验证保留新 relief JSON/奖励/称号，无需停用妖塔或自动探索 |
