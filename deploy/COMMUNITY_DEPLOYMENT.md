...
### 2026-09-19 候选迁移复核：妖塔 v0.11 与 V4 战功（尚未代表已经上线）

当前生产已有 `1700000000040-AddWordFrontV4ArcadeGames`，共41条迁移、150张 public 表。本候选新增唯一迁移 `1700000000041-AddWordFrontProgress`：只新增账号级 `word_front_progress` 表，记录服务器验真 V4 对局的独立战功、胜负、每日获取上限和三项纯展示收藏。它不读写钱包、办公币、旧成绩、妖塔存档或地图；用户注销时由外键级联删除自身进度。生产升级必须先用新鲜备份在隔离 PostgreSQL 16 副本验证 0040→0041 UP/DOWN/UP，核对 42/42 迁移、151张 public 表、新表为空且旧业务行不变，才可执行唯一生产 UP。

正常应用回退保留 schema41、战功及全部玩家数据，**不执行 DOWN、不回灌旧库**。旧 API 不查询新表，可作为应用回切基线；回切前仍要在恢复副本验证它能保留妖塔新增的未知 `seasonal` JSON 字段。候选通过备份、迁移、镜像与公网验收后，再用实际提交、镜像和时间替换本段。

### 2026-09-15 18:06 当前发布：探索回执与免费异步猜画（API / Web）

北京时间 **18:06:56** 只切换 API/Web，应用固定到 `f346f0c413d653203ae7f4c8d9ed2eadb8276e76`。API 镜像 `sha256:6f4d25ca2204c47d9581586aa71e7162e21346fe962538b7020c2d18d2af57ee`，Web 镜像 `sha256:04ea075460d0116dbc48e135e525e2afef9c01e0905f485a44a1a32bfa745eb8`；PostgreSQL、Redis、Gateway 完整容器身份、数据卷及除 IMAGE_TAG 外的环境不变。schema39 / 40 迁移 / 148 表，没有 UP/DOWN。新增完整妖塔出发回执与恢复、免费不限每日创作的异步猜画及无奖励评分、公司页深色对比修复；不是新游戏、支付或权限发布。

部署前完成新鲜备份的隔离完整恢复及离站解密校验、148 表内容/结构/序列核验和候选/桥镜像只读 40/40 迁移映射；最终镜像真 PG 58、四模块真实 HTTP 12 组/60 请求、前端 219 文件/1934 测试、类型及社区 API/Web 构建通过。完整后端基线 1626 通过另 84 条条件跳过，最终专项 206 通过。上线健康检查、公网 65 项及 104 个发行文件哈希通过；生产认证 Cookie 烟测明确跳过，合成认证 HTTP 不是生产登录或完整 community 根模块验收。18:13:16 反馈复核 33 条中 31 完成、2 进行中，两个大型提案仍 6/11、4/9。

**本次直接回退为 API + Web，不是 Web-only**：API 使用隐私兼容桥 `11eae10148766db7b64b649cde14bfd31feb5c62`，镜像 `sha256:323679fb4ecc474ba585844e3157d2d84e88c9016fc1846513f17d005c59872d`；Web 使用切换前第三阶段 `2b7b8d9c03a99cea0a7e1fd3e66f714e68baceb9` 原镜像 `sha256:3799fae5386a3eab46b61aea9e77d7675405c725369de0e50fd6297adebee91a` 和本批完整旧配置，仅改 IMAGE_TAG 指向桥。裸 832f528 不理解新评分的注销清理，不能直接回切；新桥 7 项兼容验证通过，旧业务写入保留新 JSON、注销清理目标评分。先排空并仅切 API/Web，保留全部玩家数据和 40 迁移，不执行 DOWN、回灌、授权/奖励重放、基础设施重建；API 回切会结束内存临时房。本预案未在生产执行。[发布与分层证据](../docs/RELEASE_COLLABORATION_20260915.md)。纯文档提交不再构建或部署应用。

### 2026-09-15 16:30 历史发布：六款本地小游戏第三阶段（仅 Web）

北京时间 16:30:24 仅替换 Web，生产源码与标签固定到 `2b7b8d9c03a99cea0a7e1fd3e66f714e68baceb9`，镜像 `sha256:3799fae5386a3eab46b61aea9e77d7675405c725369de0e50fd6297adebee91a`。新增六款自托管原版小游戏、低调工作稿和精选筛选，完整目录为 21 既有 + 6 新款共 27 个入口。新六款仅本轮内存，不接办公币、正式榜、本站房间或云存档；第一、第二阶段保留，不改权限/经济。API 仍为 `832f5288ea46a6018263c887865040014e56d42d` 原镜像 `sha256:8b99cf12840f6c15e59ad9f7f96d03e178a077f723de055ee246ba1072d93592`；四个非 Web 容器完整身份、数据卷及除 IMAGE_TAG 外的环境不变，schema39 / 40 迁移 / 148 表，本批无迁移。

217 文件 / 1900 前端测试、类型/社区构建、新鲜备份完整隔离恢复/离站校验通过；同一不可变镜像按 image ID 提取、全静态校验、无网络/无端口的 Nginx 实际 HTTP 头与 404 检查、本机六款匿名实操和代表窄屏检查通过。公网最终完整复跑 65 项 HTTP 及 100 文件哈希通过；认证 Cookie 烟测跳过，不冒充全认证/关卡/手机矩阵。切换期间 502 与 443 连接超时保留事实，完整第三次检查通过，不放宽安全策略或限流。本批仅新增六个确切实验室文档的本站嵌入例外且六款 connect-src none，既有遮司例外与站点外壳禁止嵌入保留；这是本批唯一已审阅可执行部署配置变化，不能作为后续任意豁免。切换前重新校验备份、候选干净提交、完整环境仅标签差分、配置及镜像 ID 防漂移。

本批直接回退仅使用第二阶段 `cd11419949b4f9cfcb949f34fddc53c69cde2451` Web 镜像 `sha256:4616269aaaf608e78b82a23dc8348e191a8289582b6035236a3016004fa1675f` 和本批切换前完整环境，只替换 Web；API/房间、数据库、Redis、网关及数据卷不动，不执行 DOWN/旧库回灌/赠送授权重放。本批没有读取反馈或恢复暂停 Palm AI。[第三阶段发布](../docs/RELEASE_GAME_PACK_20260915.md) · [玩法、源码与许可](../docs/LOCAL_GAME_LAB.md)。后续文档提交不再次构建或发布应用。

### 2026-09-15 15:12 历史发布：个人工作台与成长档案第二阶段（仅 Web）

北京时间 15:12:40 仅替换 Web，生产源码与 Web 标签固定到 `cd11419949b4f9cfcb949f34fddc53c69cde2451`，镜像 `sha256:4616269aaaf608e78b82a23dc8348e191a8289582b6035236a3016004fa1675f`。个人工作台/公开档案、五分类成长收藏、称号确认和权益/台账整理已上线；不改变经济、授予权限或新增游戏。API 继续运行原 `832f5288ea46a6018263c887865040014e56d42d` / `sha256:8b99cf12840f6c15e59ad9f7f96d03e178a077f723de055ee246ba1072d93592` 容器。schema39 / 40 条迁移 / 148 表，本批无迁移；四个非 Web 容器完整身份、数据卷与除 IMAGE_TAG 外的环境不变。

新鲜加密备份完整隔离恢复及离站/内部摘要校验、208 文件 / 1811 项前端回归、类型检查、不可变镜像预检、最终 65 项公网烟测和 18 个发行物哈希通过。超时/限流后的失败及自然窗口恢复后复跑见发布记录，不放宽配置。私有界面只在本机回环合成数据验收，不代理生产，不操作真实支持台账；生产认证 Cookie 烟测明确跳过。切换前已重验候选干净提交、完整环境仅 IMAGE_TAG 差分及 Compose 配置，不把这些门禁推迟到启动之后。仅部署说明的历史文档变化可豁免代码差分，不能豁免可执行部署文件。

本批直接回退仅使用第一阶段 `f88ba2c8af2334d419d831ce027896f5d7f5ee3f` Web 镜像 `sha256:9e00d51fa9c33bac236fef42db1f742564f2c24f93e03149d2def20753a3665d` 与本批切换前完整环境；只替换 Web，API/房间不重启，不执行 DOWN、回灌旧库或删除数据卷。[第二阶段发布与验收](../docs/RELEASE_UI_PHASE2_20260915.md) · [路线图](../docs/UI_ROADMAP_20260915.md)。后续文档提交不再次发布应用。

### 2026-09-15 14:30 历史发布：工作台视觉升级第一阶段（仅 Web）
...
当晚 API/Web 应用 `832f5288ea46a6018263c887865040014e56d42d` 发布，修复技能满熟练度说明、档案外观布局与异步猜画题库范围；当时无限次数/评价仍进行中。schema39 / 40 条迁移 / 148 表，无迁移，原基础设施不变。[当批记录](../docs/RELEASE_FEEDBACK_20260914.md)。该 API 是视觉升级前三阶段继续运行的历史后端，已被上方 f346f0c 替换；不把 d641ae0 或 832f528 当作当前 API。
...
若需回退到已验收应用 `e1838603c80aafce459cb2d2ef8a3507f4b4678c`，只切回其不可变 API/Web 镜像和旧完整环境，保留 schema37、新成绩和所有玩家数据，不执行 DOWN、不回灌旧库。新妖塔升级保底在旧应用中无法兑现；回退期间须先暂停妖塔手动写入及自动探索，待恢复认识保底状态的应用后再开放，不得直接丢弃新 JSON 状态。
...
全量前端 **189 文件 / 1622 项**通过；后端 **157 套 / 1565 项**通过，另有 5 套 / 57 项按既有条件跳过，且 e183860 相对 cfe439c 的 backend/shared 树不变。类型检查和完整构建通过；新鲜备份在隔离 PostgreSQL 16.14 恢复，候选与直接回滚 API 镜像的迁移映射均为 **37/37 已应用、0 待执行**且逐项一致，另核对 148 张表、381 个索引和 0 个未验证约束，测试资源已精确清理。真实站点壳层 Firefox 覆盖 1440 / 1280 / 390px 深浅色 6 组、42 个装备分页状态与 18 张截图：装备桌面最大文档高度 1650px（门限 1655px），20 件武器和 16 项技能全部可分页访问，未发现横向溢出或裁切。公网烟测 63 项通过；匿名浏览器首页、保护路由和 CMYK 通过。因没有获准的合成凭据，本轮未运行认证 Cookie 烟测，不能把匿名范围写成完整认证流程验收。

直接回滚基线为 `cfe439cc2936cbdc2a834cfe3a457b368487920d`，保留的 API 镜像为 `sha256:569f551b378da5ac164a4725127f21a3fecb9bb77d89593ef8f1636254625d99`，Web 镜像为 `sha256:2393403095a988725ce336983936deaba2c6df763c8c9f19feb32b13ce1bc47f`。排空普通在途操作后，仅恢复该批 API/Web 镜像和完整旧环境；保留数据库、全部新旧存档、画作、回执、资产、反馈和审计，不恢复旧 dump、不删数据、不执行 DOWN。e183860 与 cfe439c 的异步猜画服务端语义相同，因此直接回切**不需要等待 140 秒**；只有继续执行历史 cfe439c → a84759f 回退时，才先停 Web 并等待、核验两分钟草稿截止窗。回退失败恢复 e183860 镜像和环境，不套用更早妖塔暂停措施。

本批六条待审阅反馈已于北京时间 17:06 完成并复核；最终 28 条反馈中 27 条完成、0 条待审阅、1 条进行中。妖塔 v0.11 仍保持 4/11 进行中，不以六标签整合替代其余规划。

### 2026-09-11 17:02 历史发布：压力整理与统一工作区导航

当时应用 `a84759f9caae25fb15fa0ffb756af1f8ccd437ee`，北京时间 17:02:37 上线，GitHub `main` 已快进同步。新增免费活跃机会挑战、绑定解压币、真实种植经验/妖塔物资与三款称号，保留原每日巡视；统一跨页目录支持本账号当前浏览器内排序、隐藏和重置，不跨设备云同步、不改变权限或工作台收藏。[玩法规则](../docs/OFFICE_RELIEF.md) · [历史发布与回滚](../docs/RELEASE_RELIEF_NAVIGATION_20260911.md)。

无 SQL 迁移，schema36 / 37 条迁移 / 148 张表不变；只切换 API/Web、环境仅 IMAGE_TAG，PostgreSQL、Redis、Gateway 完整容器身份及生产数据卷保持不变。前端 181 文件 / 1468 项通过；后端在 `525df4e` 完成 156 套 / 1538 项，另 5 套 / 40 项条件跳过，最终提交与其 backend/shared 树完全一致。类型与构建、最终真实 PG 12 项、服务 8 组、旧 941 兼容 6 组、压力整理浏览器 13 组 / 59 张截图及后置 PG 6 组、导航 8 组 / 31 张截图（18 项布局矩阵）、公网 57 项及 5 个实际静态资源哈希均通过。7 项生产备份与 8 项合成归档摘要经本机/异机核验，5 个合成容器及独立内部网络已精确清理，证据与镜像保留。

直接回滚基线 `9413977c973e1146a9c70ef02875251e2a2ffbd2`：先停止新版 API/Web 写入、等待在途操作退出，再恢复本批旧环境与旧不可变镜像，只启动 API/Web。旧版兼容探针验证保留 relief JSON、真实种植经验、全部奖励/回执及称号所有权，**无需禁用妖塔或自动探索**。保留全部生产数据、投稿和审核，不执行 DOWN、旧库覆盖、新字段删除或权限/奖励重放；不可套用下面 941 → 342 的历史暂停步骤。

### 2026-09-11 14:41 历史发布：妖塔提案适配

当时应用 `9413977c973e1146a9c70ef02875251e2a2ffbd2`，北京时间 14:41:39 上线，GitHub `main` 已快进同步。八部位形象、当前配装战力、真实好友切磋、探索统计/符、办公币补给/阶梯箱/技能碎片与养成手册统一到原七个工作区，物资申领现为五分类。旧灵石、残魂渠道仍不扣办公币，新办公币专区明确确认后扣全站钱包，不接真实付费。[功能适配](../docs/DEMON_TOWER_PROPOSALS_20260911.md) · [历史发布与回滚](../docs/RELEASE_PROPOSALS_20260911.md)。

本次无生产迁移，schema36 / 37 条迁移 / 148 张表不变；仅切换 API/Web、环境仅 IMAGE_TAG，数据库、Redis、Gateway 容器及生产数据卷不变。前端 176 文件 / 1346 项、后端 151 套 / 1444 项通过，另有 4 套 / 28 项条件跳过；全仓类型与完整社区构建通过。最终真实 PostgreSQL 8 组、Firefox 155.0.1 的 8 组 / 41 张唯一截图（1440/390/320px 深浅色）及公网 29 项、静态哈希通过，克隆 148 表及异机证据校验通过；隔离合成验收资源已精确清理。

该次直接回滚基线 `34260773f613b540bb9fa4257b8112ccbc87d019`：**先停止新 API，再禁用旧版妖塔整体及自动探索**，保留新增 `appearance` / `provisions` JSON、全部钱包资产/流水、帖子与审计；投稿写入和审核继续开启。只回切 API/Web，不执行迁移 DOWN、旧库覆盖、新字段删除、权限或赠送重放。该暂停要求只适用于对应历史版本，不是当前 e183860 → cfe439c、历史 cfe439c → a84759f 或 a84759f → 9413977 的回滚步骤。

### 2026-09-11 11:44 历史发布：妖塔经济与统一物资申领

当时应用 `34260773f613b540bb9fa4257b8112ccbc87d019`，北京时间 11:44:14 上线。灵石、定额申领、永久/临时丹、符文与收支当时集中四分类，保留原残魂来源和办公币。无生产迁移，schema36 / 37 迁移 / 148 表不变，只切 API/Web、环境仅 IMAGE_TAG；投稿/审核保留开启，基础服务完整容器身份不变。新鲜备份恢复及异机校验、8 组真实 PostgreSQL、最终 Firefox 8 组流程 / 23 张唯一截图和公网 25 项通过。[历史发布与回滚](../docs/RELEASE_ECONOMY_20260911.md)。

该次回滚基线为 `2111004`：先停止新版 API 再暂停妖塔整体与自动探索，保留新经济 JSON、资产、帖子和审计；投稿/审核仍开启。不得回灌旧库、删除经济字段或重放该次协作授权。

### 2026-09-10 20:30 历史发布：成员投稿与站长审核

当时应用 `2111004be1cc407b95cc9859afc43ac8fb10ece5`，北京时间 20:30:17 上线。正常账号投稿、站长审核后展示；无数据库迁移，schema36 / 37 迁移 / 148 表不变。环境仅 IMAGE_TAG 与内容写入/审核两开关变化，其他配置和基础设施 ID 不变。zbrs 沿用已有 admin 角色，没有授予真实账号新权限或核验状态。备份恢复、最终镜像 6 组工作流 / 14 张截图与公网 25 项验收通过。[发布与回滚](../docs/RELEASE_POSTING_20260910.md)。

该次回滚基线为 `49071c8`，使用该批旧环境关闭内容写入和审核，保留全部新增帖子与审计数据，仅切 API/Web；不回灌旧库或重放权限。下文为历史快照。

### 2026-09-10 18:40 历史发布：工作台深色模式

当时应用 `49071c8452e1749423d3b58c88c8c59677a54d03`，北京时间 18:40:30 上线。仅前端，新增浅/深/跟随系统主题、本机记忆和跨标签同步；schema36 / 37 迁移 / 148 表不变。新鲜备份恢复和异机校验、最终镜像 8 组浏览器 / 33 张截图、公网 20 项及 Chromium 系统主题/窄屏验收通过。只切 API/Web，环境仅 IMAGE_TAG，基础设施 ID 不变。[发布与回滚](../docs/RELEASE_DARK_MODE_20260910.md)。

该次回滚基线为 `7dd04d2`，保留 schema36 与本机数据，旧客户端忽略主题偏好键即可。不执行迁移、旧库覆盖、赠送或权限重放。下文为历史快照。

### 2026-09-10 17:55 历史发布：工作台统一界面

当时应用 `7dd04d23a0e094e8e542d46bb54feb80a2d92eea`，北京时间 17:55:17 上线。该次仅前端，统一样式与栏目目录；schema36 / 37 迁移 / 148 表不变。新鲜备份恢复及异机校验、最终镜像 8 组浏览器 / 31 张截图和公网 19 项验收通过。仅切 API/Web，环境只改 IMAGE_TAG，基础设施 ID 不变。[发布与回滚](../docs/RELEASE_INTERFACE_20260910.md)。

该次回滚基线为 `57345dd`，认识 schema36 和分离存储的桌宠图片；不执行迁移、旧库覆盖、权限赠送或账号注销关停。下文 17:04 及以前为历史快照。

### 2026-09-10 17:04 历史发布：我的工作台与桌宠增强

当时应用 `57345dd5dbe510fc74f771e887f61afaa1ef8816`，北京时间 17:04:44 上线。前端与只读每日钱包汇总接口，schema36 / 37 迁移 / 148 表不变，未执行迁移或赠送。新鲜备份恢复、异机校验、真实 PostgreSQL、最终镜像 12 组浏览器/17 张截图和公网 13 项验收通过；只切 API/Web，环境仅 IMAGE_TAG，基础设施 ID 保持不变。[正式发布及回滚](../docs/RELEASE_WORKSPACE_20260910.md)。

该次回滚基线为 `4840700`，保留数据库与分离存储的本机图片；旧客户端可能暂时显示内置形象，不删除新图片键、不关账号注销，不套用更早 f46 的回退步骤。下文 14:35 及以前是历史快照。

### 2026-09-10 14:35 历史发布：免费工位搭子

当时应用 `48407006c70c5964c13ffd15e2670a322664344e`，北京时间 14:35:41 上线；该次仅前端改动，schema36 / 37 条迁移 / 148 表保持不变，不执行 UP/DOWN。新鲜备份完整恢复、候选映射与无待执行迁移、全部旧数据/结构/序列比对、异机校验及最终浏览器/公网验收均通过。只替换 API/Web，环境仅 `IMAGE_TAG` 变化，数据库/Redis/网关不动。详见[工位搭子正式发布](../docs/RELEASE_DESK_PET_20260910.md)。

该次回滚基线 `754dcaf083b3841a7e0dbc77e92bd1b6c0b79cc4` 是 14:24:58 上线的桌宠初版，也认识 schema36；保留数据库和本机桌宠数据，不执行恢复覆盖，不沿用更早 f46 回退的账号注销关停。下面 11:48 与 f46 的段落仅为历史快照。
...
以下历史状态来自 [Paper v2 正式发布记录](../docs/RELEASE_PAPER_V2_20260909.md)，该次发布后复核时间为北京时间 **2026-09-09 21:58**；不是当前运行版本：
...
| 当时运行应用 | `f46a765271a55d5708c6f6e0299c149b4183ef11`；2026-09-09 21:54:52 上线 |
...
开发协作 AI 只接 Groq 免费层的固定 `openai/gpt-oss-20b` 模型。先在 Groq 控制台创建免费项目密钥，再将 `GROQ_API_KEY` 写入限权 `.env.community`，最后设置 `FEATURE_DEVELOPMENT_AI_ENABLED=true`；预检会在开关开启但密钥缺失时停止发布。服务端不提供付费模型回退，站内限制为每账号每日 20 次、全站每日 200 次。提案正文和最近人工评论仅在用户显式勾选知情后发送，附件正文不发送，回答不写入正式时间线，也不能修改状态、提交或部署。密钥不得进入 Git、前端构建、日志或浏览器响应。

## 3. 已运行社区站点：普通应用更新

以下是**后续获授权应用发布**的检查顺序；当前发布的实际结果以上方快照和正式发布记录为准。不要照抄空站的全栈 `up`、旧迁移、成员授权或 VIP 发放步骤。
...
| --- | --- | --- |
| `cfe439c` | schema36 → 36，无迁移 | [本批协作反馈适配](../docs/RELEASE_COLLABORATION_20260912.md)；当时 e183860 的直接回滚基线，认识异步猜画草稿；e183860 → cfe439c 不需要等待 140 秒 |
| `a84759f` | schema36 → 36，无迁移 | [压力整理与统一工作区导航](../docs/RELEASE_RELIEF_NAVIGATION_20260911.md)；历史 cfe439c 的回滚基线，cfe439c → a84759f 须使用两分钟草稿截止窗预案 |
| `9413977` | schema36 → 36，无迁移 | [妖塔提案适配](../docs/RELEASE_PROPOSALS_20260911.md)；历史 a84759f 的回滚基线，兼容探针验证保留新 relief JSON/奖励/称号，无需停用妖塔或自动探索 |
| `3426077` | schema36 → 36，无迁移 | [灵石经济与统一物资申领](../docs/RELEASE_ECONOMY_20260911.md)；仅是历史 9413977 的回滚基线，该次回退须保留新增 JSON 并暂停旧版妖塔/自动写入 |
| `2111004` | schema36 → 36，无迁移 | [成员投稿与站长审核](../docs/RELEASE_POSTING_20260910.md)；其关闭投稿/审核的旧回退预案不适用于当前妖塔发布 |
| `49071c8` | schema36 → 36，无迁移 | [深色模式](../docs/RELEASE_DARK_MODE_20260910.md)；本机主题偏好与业务数据保留 |
| `ff6bab4` | schema32 → 35，新增 0033/0034/0035 | [协作补齐与红蓝房间](../docs/RELEASE_COMPLETION_20260909.md)；仅作为历史 f46 的回滚基线。该次回退到 a456 的特殊关停条件不套用到 f46 → ff6 |
