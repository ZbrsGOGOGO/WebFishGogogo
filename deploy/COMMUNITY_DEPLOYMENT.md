...
217 文件 / 1900 前端测试、类型/社区构建、新鲜备份完整隔离恢复/离站校验通过；同一不可变镜像按 image ID 提取、全静态校验、无网络/无端口的 Nginx 实际 HTTP 头与 404 检查、本机六款匿名实操和代表窄屏检查通过。公网最终完整复跑 65 项 HTTP 及 100 文件哈希通过；认证 Cookie 烟测跳过，不冒充全认证/关卡/手机矩阵。切换期间 502 与 443 连接超时保留事实，完整第三次检查通过，不放宽安全策略或限流。仅六个确切游戏文档允许本站嵌入且 connect-src none，其余外壳仍禁止嵌入；这是本批唯一已审阅可执行部署配置变化，不能作为后续任意豁免。切换前重新校验备份、候选干净提交、完整环境仅标签差分、配置及镜像 ID 防漂移。
