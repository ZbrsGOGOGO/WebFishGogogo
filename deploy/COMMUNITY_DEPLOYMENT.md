...
迁移清单新增 `1700000000037`，由 schema36 的 37 条迁移变为 schema37 的 38 条迁移。此迁移只扩展 `arcade_game_runs`、`arcade_best_scores` 两张表的游戏键 CHECK，允许独立的 `word_story` / `word_endless` 成绩；不改旧行、不增加表、不修改办公币。须先对生产新鲜备份做隔离恢复与准确的 36→37 增量演练，确认旧行内容及 148 张表不变，再运行一次 `migrate`，随后仅切换 API/Web。此候选及以下迁移说明不表示已经上线；上线结果另记发布记录。
