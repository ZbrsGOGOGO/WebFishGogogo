...
本次增量版本增加服务端小游戏房间/每日奖励榜（`1700000000027`）和按来源独立保存的每日热榜快照（`1700000000028`），迁移目标为 `1700000000028`。仅增加新表，不改旧账户、钱包余额、聊天记录、存档或开发附件；发布要验证 `0026 → 0028` 以及新表为空时的隔离回滚。小游戏规则见 [小游戏房间](../docs/COMMUNITY_GAME_ROOMS.md)。私有开发协作 `1700000000026` 已有数据，保留原开关与授权，不重复执行旧成员授权步骤。协作附件随数据库备份，不启动无人值守 AI 监控。使用与权限说明见 [开发协作台](../docs/DEVELOPMENT_WORKSPACE.md)。
...
- 每次升级均必须完整登记至最新迁移 `1700000000028`：账号安全 `0013`、聊天室 `0014`、新闻 `0015`、热点索引 `0016`、用户名账号 `0017`、游戏成长字段 `0018`、统一等级/体力/货币 `0019`、帮派基础 `0020`、共享帮派首领 `0021`、每日热点与邀请币 `0022`、小游戏排行榜与聊天留存 `0023`、好友私聊 `0024`、Arcade 的 `zhesi` game key `0025`、私有开发协作 `0026`、权威小游戏房间/日榜/奖项 `0027`、来源独立热榜快照 `0028`；
- 先在空新表的临时数据库分别回退 `0028` 和 `0027`，验证两张快照表及五张游戏表删除，然后继续旧 `0026/0025` 检查。真实生产仅作应用回退，绝不删新表或回收已发奖资产。
...
- 逐个回滚到 `0007` 后，上述 `0013`—`0028` 的表、字段、索引、约束和相关迁移记录必须全部消失，再次升级必须重新完整创建；
...
本次 `0026 → 0028` 属于社区版增量更新，不执行下面的 public 切站流程。发布前保存旧 community 镜像、完整提交 SHA、限权环境文件和数据库备份。候选通过隔离数据库验收后，仅运行 `compose run --rm --no-deps migrate`；验证最新迁移、旧用户/权限/钱包/反馈数据不受迁移影响，再用 `--no-deps` 替换 API/Web。应用回退时恢复旧 community 发布配置和 API/Web，原开发协作开关及成员权限保持不变；保持 PostgreSQL、Redis、Gateway 及其数据卷不变。保留 `0027/0028` 新表、已发放资产及已有 `0026` 提案附件，不在生产执行 `migration:revert`。
