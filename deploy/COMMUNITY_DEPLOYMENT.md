...
候选迁移 `1700000000038` 仅扩展 `arcade_game_runs` 与 `arcade_best_scores` 的游戏键 CHECK，允许独立的 `word_story_v2` / `word_endless_v2` 成绩；不删除旧行、不改变既有办公币和权限。生产当前仍为下面记录的 schema37 / 38 条迁移；须先用新鲜备份的隔离恢复副本验证 0038 唯一增量、旧数据不变及回滚桥兼容，才可执行生产 UP。不能对生产运行 DOWN 或回灌备份。候选新技能 s17/s18 及 v2 成绩不被原 `9013da7` 完整识别，紧急应用回退只能使用已单独验证的兼容桥 `1f50c07f543d51ffa6688d90ff87b12b805ff921` 的 API/Web 不可变镜像，不得直接退到裸 `9013da7`。最终镜像、提交、备份和验收以本批正式发布记录为准，准备阶段不宣称已部署。
...
| --- | --- | --- |
| `cfe439c` | schema36 → 36，无迁移 | [本批协作反馈适配](../docs/RELEASE_COLLABORATION_20260912.md)；当前 e183860 的直接回滚基线，认识异步猜画草稿；e183860 → cfe439c 不需要等待 140 秒 |
