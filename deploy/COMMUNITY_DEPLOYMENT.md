...
本次待发布版本新增私有开发协作模块，迁移目标为 `1700000000026`。`FEATURE_DEVELOPMENT_WORKSPACE_ENABLED` 默认关闭；确认站长账号后可启用 `/development`。附件存于私有 PostgreSQL 字段，会进入数据库备份；本模块不启动 AI 任务或自动部署。使用与权限说明见 [开发协作台](../docs/DEVELOPMENT_WORKSPACE.md)。

社区版是独立于 `review`、`public` 和旧 `full` 站的发布模式。它启动 PostgreSQL、Redis、数据库迁移、`main.community.js` API、社区 SPA、Nginx 和 Caddy。Nginx 只代理已实现且明确列入白名单的认证、账号、关系、绿植、内容、审核和 Arcade API；旧文档上传、私人阅读、便签、偏好、工具目录和已停服的办公室乐斗 API，以及尚未实现的社区前缀一律返回 404。“摸鱼升职记”工位塔防 V1 是纯前端短局，不新增服务端游戏 API；“遮司”由 `/games/zhesi` React 页面承载、复用 `/games/zhengdao/` 静态 iframe 资源，并以 `zhesi` game key 接入现有 Arcade 赛局和排行榜。
...
- 每次升级均必须完整登记至最新迁移 `1700000000026`：账号安全 `0013`、聊天室 `0014`、新闻 `0015`、热点索引 `0016`、用户名账号 `0017`、游戏成长字段 `0018`、统一等级/体力/货币 `0019`、帮派基础 `0020`、共享帮派首领 `0021`、每日热点与邀请币 `0022`、小游戏排行榜与聊天留存 `0023`、好友私聊 `0024`、为 Arcade 两张表放行 `zhesi` game key 的 `0025`，以及私有开发协作的 `0026`；
...
- 逐个回滚到 `0007` 后，上述 `0013`—`0026` 的表、字段、索引、约束和相关迁移记录必须全部消失，再次升级必须重新完整创建；
- `trim/lower` 后邮箱冲突必须在 schema 变更前中止；
- `user_profiles` 表锁竞争必须在 `lock_timeout` 内失败，并回滚此前已执行的 `users` DDL/数据更改；
- 释放锁后同一快照可正常升级。

只有脚本返回 0、输出最终 passed，且日志与镜像 SHA 一起归档，才可放行迁移。本轮只准备脚本，未执行容器演练；这是 Linux 发布环境必须补齐的显式门禁，不得把静态检查写成“迁移已通过”。
...
本次 `0025 → 0026` 属于社区版增量更新，不执行下面的 public 切站流程。发布前保存旧 community 镜像、完整提交 SHA、限权环境文件和数据库备份。应用回退时恢复旧 community 发布配置，关闭 `FEATURE_DEVELOPMENT_WORKSPACE_ENABLED`，仅替换 API/Web 并验证健康；保持 PostgreSQL、Redis、Gateway 及其数据卷不变。保留已升级的 `0026` 表，不执行 `migration:revert`，避免删除真实提案与附件。
