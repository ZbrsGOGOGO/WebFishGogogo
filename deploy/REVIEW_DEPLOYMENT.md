...
- `SITE_OPERATOR`：与备案申请完全一致的主办者名称；
- `SITE_CONTACT`：可公开的邮箱或电话；
...
健康检查应返回 `mode: review`，业务 API 应返回 `404`。

## 3. 接入域名与 HTTPS

继续保持容器入口只监听 `127.0.0.1:8080`。使用宿主机 Nginx/Caddy 或云负载均衡为真实域名终止 TLS，再反向代理到该地址。只开放 SSH、80 和 443，不要公开 8080。

上线后检查：首页、隐私政策、服务条款、HTTPS 证书、HTTP 跳转、备案状态、主办者与联系方式；并确认 `/api/`、`/login`、`/register`、`/games`、`/library` 均无法使用。
