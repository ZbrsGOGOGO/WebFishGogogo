...
> **不是当前网站的部署流程。** 本文保留旧 `full` 模式的 PostgreSQL、API、Worker 和文档正文卷部署说明；当前摸摸公司运行的是独立 `community` 模式，请从 [部署导航](README.md) 或 [社区部署手册](COMMUNITY_DEPLOYMENT.md) 开始。
>
> 下文的 `.env.production`、根目录 Compose、`backup.sh`、`restore.sh`、Worker 以及全栈启动/停机命令只供旧环境维护参考。不要在 `/opt/webfish-review` 的现有社区生产站原样执行，不要用旧 full 恢复脚本覆盖社区数据库。历史版本、依赖和容量描述不是当前社区版的验收结论；本次文档与主分支整理不执行这些命令。
...
该历史全栈启动流程会先执行尚未运行的数据库迁移。应用回退与灾难恢复是不同操作：优先核验旧应用能否兼容现有 schema，不得默认用旧备份覆盖发布后数据。确需恢复数据库和文档卷时，必须另行确认影响范围并采用已演练的恢复方案；不要直接删除持久卷。该旧方案为单机停机发布，没有自动灰度或自动回滚。
...
`down` 会删除该项目的容器和网络，通常保留命名卷，但不是当前社区站常规更新或回滚步骤。不要在生产环境执行 `docker compose down -v`。
