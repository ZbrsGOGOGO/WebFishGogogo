...
const oldTimestamp = 1700000000026;
...
assert.ok(migrations.some((migration) => timestamp(migration) === 1700000000027), 'Candidate migration 0027 required');
assert.ok(migrations.every((migration) => timestamp(migration) <= 1700000000028), 'Review this rehearsal before running future migrations');
const options = {
...
async function schemaRehearsal() {
  ...
  for (;;) {
    ...
    assert.ok([1700000000027, 1700000000028].includes(Number(last.timestamp)));
  ...
  check('migration 0026 → candidate 0027/0028 → 0026 → candidate, no baseline changes');
}
...
async function roomChecks() {
  ...
  const invitation = await play.create(people[10].id, request('draw', 'room', { visibility: 'invite' }));
  ...
  assert.equal((await play.join(people[11].id, { code: invitation.joinCode.toLowerCase() })).id, invitation.id);
  ...
  const left = await play.leave(guesser.id, running.id); assert.equal(left.game, null); assert.equal(left.joinCode, '');
  ...
  check('private invitations, block privacy, actual concurrent capacity, host readiness, real draw secrecy and forfeit exclusion');
