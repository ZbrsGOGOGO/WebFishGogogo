...
> 发布硬闸门：在公开 `/ledou` 或继续公开现有网页游戏前，必须取得网络游戏出版审批批文，或保存主管部门/专业律师关于无需另行审批的书面结论。ICP备案或一般网站审核不能替代该确认。未完成时，只能运行本地预览，不得执行本文的生产部署步骤。

正式公开版只运行 Caddy 与静态 Web 容器，开放办公室主题九系统产品地图、办公室乐斗本机试玩、11 款浏览器本地工具和四款单机游戏。九系统中当前只有首页与乐斗可进入；热点新闻、经验交流、农场、投喂、邀请、我的主页和好友仅展示产品方向，不开放独立页面或互动能力。正式公开版不启动数据库或业务 API，也不开放注册、登录、上传、午休竞技场、比大小或用户互动功能。

当前公开工具白名单为：下班倒计时、计时器、日期计算、计算器、手动汇率换算、单位换算、文本整理、字数统计、JSON 格式化、时间戳转换和颜色转换。工具输入只在当前浏览器内处理；办公室乐斗的职业、装备与试玩进度以及部分单机游戏记录只保存在浏览器本地，不上传到本站服务器。

为使隐私政策中的保存期限与部署实现一致，正式公开版不持久化常规应用访问日志：`web` 与 `gateway` 的 Docker 日志驱动均为 `none`，Nginx 全站关闭访问日志，因此本站应用日志的保存期限为 0 天。域名解析、证书签发、网络运营商或云基础设施提供者依法独立处理的数据不属于本站应用日志。今后如启用持久日志，必须先配置可自动执行的时间上限，并同步更新隐私政策、预检与运维说明。
...
- `PRIVACY_PROCESSOR_NAME`：依法可公开的个人信息处理者名称；只显示在隐私政策，不显示在首页；
- `PRIVACY_CONTACT`：真实可用的专用隐私邮箱、HTTPS 请求页面或电话；不应复用不愿公开的私人邮箱；
- `PUBLIC_GAME_CLEARANCE_REFERENCE`：游戏出版批文号，或主管部门/专业律师书面结论的内部文件索引；不得填写 ICP 备案号；
...
正式公开容器的日志驱动为 `none`，因此不要把 `docker compose logs` 作为上线验证手段。`ps --all` 应显示服务处于运行/健康状态，健康检查应返回 `{"status":"ok"}`。随后从公网验证正式域名；以下示例使用当前站点域名：
...
- 首页、办公室乐斗、工具、游戏中心、四款游戏、隐私政策和服务条款均返回 `200`；
- `/api/health` 返回 `404`；
- 登录、注册、文档库、农场、博客、竞技场和比大小入口返回 `302`，且没有使用 `curl -L` 跟随跳转；
- HTTP 跳转到 HTTPS，`www` 跳转到主域名，证书有效；
- 页面底部展示真实 ICP 备案号；工具输入、办公室乐斗与游戏过程只在浏览器内运行；
- 首页展示首页、热点新闻、经验交流、农场、乐斗、投喂、邀请、我的主页和好友九个系统，其中只有首页与乐斗提供可点击的系统入口，其余项目没有独立公开路由。

最后使用桌面端和手机尺寸各完成一次实际操作：完成一局办公室乐斗、打开一个工具、启动四款游戏、刷新 `/ledou` 与游戏深链，并确认未完成系统不能进入独立页面，也不存在可用的注册、上传、农场或午休竞技场功能路由。
