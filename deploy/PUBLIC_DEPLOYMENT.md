...
> 发布硬闸门：在公开 `/tower-defense` 或继续公开现有网页游戏前，必须取得网络游戏出版审批批文，或保存主管部门/专业律师关于无需另行审批的书面结论。ICP备案或一般网站审核不能替代该确认。未完成时，只能运行本地预览，不得执行本文的生产部署步骤。

正式公开版只运行 Caddy 与静态 Web 容器，开放摸摸公司的摸鱼升职记（工位塔防）、11 款浏览器本地工具和两款经典小游戏（俄罗斯方块、坦克大战）。

当前公开工具白名单为：下班倒计时、计时器、日期计算、计算器、手动汇率换算、单位换算、文本整理、字数统计、JSON 格式化、时间戳转换和颜色转换。工具输入只在当前浏览器内处理；工位塔防只把本机最高分与设置保存在浏览器本地，不上传成绩，不接入正式排行榜也不提供正式奖励。
...
- 首页、工位塔防、工具、游戏中心、两款游戏、隐私政策和服务条款均返回 `200`；`/ledou` 与 `/battle` 作为兼容地址亦可达，并由 SPA 跳转到 `/tower-defense`；
- `/api/health` 返回 `404`；
- 登录、注册、文档库、农场、博客、竞技场和比大小入口返回 `302`，且没有使用 `curl -L` 跟随跳转；
- HTTP 跳转到 HTTPS，`www` 跳转到主域名，证书有效；
- 页面底部展示真实 ICP 备案号；工具输入、工位塔防过程与本机成绩只在浏览器内运行；
- 首页展示工位塔防入口，不再展示乐斗作为当前产品；历史地址仅用于兼容跳转。

最后使用桌面端和手机尺寸各完成一次实际操作：用“工位守卫”完成一局工位塔防、确认刷新后本机最高分与设置保留、打开一个工具、启动俄罗斯方块和坦克大战，并验证 `/ledou` 与 `/battle` 会进入 `/tower-defense`。
