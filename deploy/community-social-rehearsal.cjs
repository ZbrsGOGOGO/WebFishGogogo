...
assert.equal(process.env.DB_HOST, 'growth-pg-btpam6', 'Reviewed isolated PG host only');
...
assert.equal(redisUrl.hostname, 'growth-redis-btpam6', 'Reviewed isolated Redis host only');
...
assert.ok(redisUrl.password, 'Dedicated isolated Redis password required');
...
assert.ok(migrations.some((migration) => Number(migration.name.slice(-13)) === 1700000000032));
assert.ok(migrations.every((migration) => Number(migration.name.slice(-13)) <= 1700000000032));
