...
assert.ok(process.env.DB_HOST === '127.0.0.1' || /^rail-rehearsal-pg-[a-z0-9]{6,16}$/.test(process.env.DB_HOST || ''), 'Explicit isolated DB_HOST required');
...
assert.ok(migrations.every((migration) => timestamp(migration) <= 1700000000029), 'Review rehearsal before future migrations');
...
async function schema() {
  ...
  check('guarded empty-business database migrates to 0029 and reruns idempotently');
}
