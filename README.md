...
截至 **2026-09-19**，仓库以 `main` 为主线，采用 `community` 前后端入口。原 `feat/workstation-tower-defense` 分支保留历史，不删除、不重写提交。

- **当前 API 为 `ef93db1`，Web 为 `3e32257`**。2026-09-19 16:16:09（北京时间）完成文字战线 V4、跨 API 重启恢复的双人房、站长地图草稿、妖塔双魂珠及最终 Web 路由修复；两个 Web 补丁不改后端代码。[发布与验收记录](docs/RELEASE_WORD_FRONT_V4_20260919.md)
- **数据库：schema `0040`**，41 条迁移、150 张 public 表；只执行一条已验证的增量迁移，PostgreSQL、Redis、网关及生产数据卷保持不变。反馈于 2026-09-19 16:21:13 复核为 34 条中 31 已完成、3 进行中、0 待审阅；新重构、妖塔 v0.11 与旧塔防提案分别为 9/10、7/11、7/9，不代表三份完整规划全部完成。
- [六款小游戏第三阶段](docs/RELEASE_GAME_PACK_20260915.md)、[文字战线 v3 与免费双人房](docs/RELEASE_WORD_FRONT_V3_20260914.md)、首页/导航、档案、投稿、成长和桌宠能力保留；小游戏仍共 27 个入口，11 款工具与 44 项成就不变。[分阶段路线图](docs/UI_ROADMAP_20260915.md) · [新候选游戏与足球房间评估（尚未接入）](docs/GAME_CANDIDATES_20260915.md)
- `main` 已同步应用代码；后续文档提交不等于应用重建。生产继续按已验收的完整提交 SHA 固定镜像，不跟随分支自动重启。[历史纸上突围发布](docs/RELEASE_PAPER_V2_20260909.md)保留追溯。
...
| 模块 | 已实现能力 | 主要入口 |
| --- | --- | --- |
| 首页与统一导航 | 真实账户状态、按需交互 3D 工位、五组目录、快捷导航与 Ctrl/⌘ K 搜索，保留个人目录偏好 | `/`、跨页栏目目录 |
| 投稿与审核 | 正常账号保存草稿、提交/撤回审核；站长审核后展示、版本与审计记录，待审修订保持私有 | `/community`、`/community/new`、`/moderation` |
| 我的工作台与账号 | 保存的身份与称号、账户余额和今日收支、成长/权益、常用与最近入口；资料编辑默认折叠，账号安全与隐私集中切换 | `/me`、`/account/security` |
...
| 小游戏与玩家房间 | 21 既有 + 6 新款共 27 个入口，单机/房间/养成/本地练习/新接入精选筛选与搜索，明确设备、进度与参榜边界 | `/games`、`/games/rooms`、`/games/lab/:slug` |
...
| 开发协作 | 授权成员提交建议和附件、讨论、版本记录、审核与完成状态；可选接入 Groq 免费 AI 讨论 | `/development` |

社区工作区的统一目录覆盖工具、游戏与公开页面，小游戏和工具可直接切换；支持本账号在当前浏览器内排序、隐藏和重置，不跨设备云同步，也不改原工作台收藏。隐藏模块仍可从完整目录找回，账号权限和开关始终由服务端控制；开关、折叠目录不重开当前游戏，实际跳页仍按原游戏离页规则处理，工作便签遮罩边界保留。关闭的模块不会伪造成功或演示数据。

压力整理每 30 分钟新认可活跃时间积累一次机会，最多保留 10 次，每日最多认可 4 小时；不按旧历史回填。解压币只用于本模块，不兑换办公币。种植礼包实际增加 30 种植经验并自然升级，不增加已停用的旧农场币；所有联动物品明确领取，条件不足时保留。原 30 秒每日巡视的 20 办公币与 5 职场经验独立保留。[免费玩法与安全边界](docs/OFFICE_RELIEF.md)

妖塔旧灵石、残魂申领渠道仍不扣办公币；「物资申领 → 办公币补给」使用全站真实钱包，明确标价并二次确认后才扣币，不是真实货币付费。配装战力只是参考分，外观不影响战斗；好友与公开论道共享每日次数，不发普通经验或办公币。原等级、共享首领与通道建设保留，不使用模拟人数卡关。界面已统一为六个工作区；大型提案未全部完成。[规则说明](docs/DEMON_TOWER_PROPOSALS_20260911.md) · [工作区整合](docs/RELEASE_COLLABORATION_20260912.md)
...
- **新接入精选**：云架构守关（生存/25关/沙盒）、麻将奇旅（24轮消除与构筑）、六角叠叠、四子连线（AI/同屏双人）、环域突围（街机生存）及星际开拓（太空经营）。无需登录，主动加载，默认静音，小窗/放大与便签暂停，**只有本轮内存进度，不接办公币、正式榜或本站房间**。Radius 需要桌面键鼠，in ASCENT 建议桌面/横屏放大。[玩法、源码与设备边界](docs/LOCAL_GAME_LAB.md)

纸上突围联机、2048、Underrun 和六款新实验室游戏 **不发办公币、不进入正式奖励榜**。原六款 Play 游戏的单人挑战与玩家房间共享各自日榜，昨日冠军按规则发奖；办公币余额榜本身不发奖。不同玩法的奖励上限分别见 [小游戏](docs/COMMUNITY_GAME_ROOMS.md)、[正式塔防](docs/WORKSTATION_CAMPAIGN_AND_PAPER_ARENA.md) 和 [妖塔](docs/DEMON_TOWER_PLAYER_GUIDE.md) 说明。

游戏小窗支持收起或工作便签遮罩。**遮罩不暂停服务器比赛，也不隐藏浏览记录或网络访问。** 纸上突围联机暂不包含原单机的钩索、场景破坏和补给拾取。

## 技术与目录

采用 npm workspaces 管理的 TypeScript monorepo：React 18 + Vite 前端、NestJS 后端、共享类型与游戏引擎；Three.js 用于纸上突围和按需首页工位等 3D 画面。
...
- 发布按完整提交 SHA 固定镜像。先验证备份可完整恢复并完成异机校验，再只更新授权服务；纯前端采用 Web-only，不重启 API/房间。保留回滚镜像，不重建数据库、Redis、网关或数据卷。
