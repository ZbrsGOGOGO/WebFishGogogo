...
截至 **2026-09-11**，仓库以 `main` 为主线，采用 `community` 前后端入口。原 `feat/workstation-tower-defense` 分支保留历史，不删除、不重写提交。

- **当前已上线应用为 `a84759f`**，2026-09-11 17:02:37（北京时间）发布免费「压力整理」与统一工作区导航：活跃机会、独立解压币、真实种植经验/妖塔奖励和稀有称号；跨页目录支持本浏览器内按账号排序、隐藏及重置。[发布与验收记录](docs/RELEASE_RELIEF_NAVIGATION_20260911.md) · [压力整理规则](docs/OFFICE_RELIEF.md)
- **数据库：schema `0036`**，37 条迁移、148 张 public 表；本次无迁移，仅替换 API/Web 与 IMAGE_TAG，数据库、Redis、网关容器保持不变。原有[投稿与审核](docs/RELEASE_POSTING_20260910.md)、[深色模式](docs/RELEASE_DARK_MODE_20260910.md)、工作台、成长和桌宠能力保留。
