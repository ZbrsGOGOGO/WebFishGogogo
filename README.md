...
> 版本边界：线上站点 <https://zbrshyyzxx.top> 已运行“摸摸公司 / 工位塔防”基线；当前仓库新增的账号安全、好友实时私聊和小游戏内容仍是 **待部署版本**，只有完成数据库迁移、发布和线上烟测后才能视为同步。

> 范围边界：本提交只落地“唯一角色 + 三类塔 + 三波敌人”的短局核心。放置离线收菜、升职成长和公司社交是后续路线，本版本不宣称已实现。
...
| 摸鱼升职记 | 一名“工位守卫”与订书机、咖啡机、打印机三类办公用品塔共同抵御三波任务 | 主玩法为工位塔防；纯浏览器短局，只在本机保存最高分 |
...
“代码中存在页面或接口”不等于“生产已经开放”。生产部署模板默认关闭注册、写入、内容、审核、聊天室和新闻后台等发布闸门；只有依赖、供应商、数据迁移和安全验收完成后才应逐项开启。工位塔防 V1 不依赖服务端游戏闸门，也不得调用已退役的乐斗结算接口。关闭状态下的其他页面必须给出真实空态、受限态或不可用态，不能伪造数据和成功结果。
...
- 工位塔防 V1 包含唯一角色、三类塔、三波敌人、建造/升级/出售、暂停/重开、键盘和移动端操作。
...
- 原有工具页，以及贪食蛇、俄罗斯方块、坦克大战和《证道 · 命格模拟》四款小游戏；浏览器本地记录不会直接写成社区正式资产。
...
前端有对应的 `VITE_COMMUNITY_*` 构建变量。生产 Compose 会把有服务端写入的功能映射到同一发布决策；前端开关只控制入口和交互，不是安全边界。工位塔防 V1 的入口使用 `VITE_COMMUNITY_TOWER_DEFENSE_ENABLED`；它是静态前端能力，不得复用 `FEATURE_COMMUNITY_BATTLE_ENABLED` 的旧服务端语义。
...
| [《摸鱼升职记》工位塔防 V1](docs/WORKSTATION_TOWER_DEFENSE_V1.md) | 单角色、三类办公用品塔、三波敌人、本机存储与验收边界 |
| [《证道》导入说明](docs/ZHENGDAO_GAME_IMPORT.md) | 用户提供静态游戏的来源哈希、安全裁剪、本机存档与发布权利门禁 |
