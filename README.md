...
截至 **2026-09-14**，仓库以 `main` 为主线，采用 `community` 前后端入口。原 `feat/workstation-tower-defense` 分支保留历史，不删除、不重写提交。

- **当前已上线应用为 `9013da7`**，2026-09-14 09:42（北京时间）发布独立的「文字战线·赵云救阿斗」PvE 首期、服务端校验的两张独立榜，以及妖塔升级掉落/武器里程碑和战报展示；原工位塔防、存档与办公币规则保留。整份提案尚未完成。[发布与验收记录](docs/RELEASE_WORD_FRONT_20260914.md)
- **数据库：schema `0037`**，38 条迁移、148 张 public 表；本次只扩展 Arcade 游戏键约束，仅替换 API/Web 与 IMAGE_TAG，PostgreSQL、Redis、网关容器保持不变。原有[投稿与审核](docs/RELEASE_POSTING_20260910.md)、[深色模式](docs/RELEASE_DARK_MODE_20260910.md)、工作台、成长和桌宠能力保留。
...
| 文字战线 | 独立的赵云救阿斗文字塔防首期：五字卡招募、双字角色、同阶合并、三章剧情与 30 波无尽；服务端回放核验的独立成绩，不发办公币 | `/tower-defense/word-front` |
...
妖塔旧灵石、残魂申领渠道仍不扣办公币；新增「物资申领 → 办公币补给」使用全站真实钱包，明确标价并二次确认后才扣币，不是真实货币付费。配装战力只是参考分，外观不影响战斗；好友与公开论道共享每日次数，不发普通经验或办公币。原等级、共享首领与通道建设保留，不使用模拟人数卡关。[规则说明](docs/DEMON_TOWER_PROPOSALS_20260911.md)
