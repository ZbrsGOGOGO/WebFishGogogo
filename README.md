...
截至 **2026-09-15**，仓库以 `main` 为主线，采用 `community` 前后端入口。原 `feat/workstation-tower-defense` 分支保留历史，不删除、不重写提交。

- **当前 API / Web 均为 `f346f0c`**。2026-09-15 18:06:56（北京时间）发布妖塔最近出发回执与刷新恢复、免费不限每日次数的异步猜画及 1–5 分互评，并修复公司页深色背景对比。评分不发奖励，不改变妖塔概率、体力或办公币规则。[发布与验收记录](docs/RELEASE_COLLABORATION_20260915.md)
- **数据库：schema `0039`**，40 条迁移、148 张 public 表；本次无迁移，仅更新 API / Web，PostgreSQL、Redis、网关及生产数据卷保持不变。反馈实时复核为 33 条中 31 已完成、2 进行中（北京时间 18:13:16）；妖塔 v0.11 与塔防大更改仍分别为 6/11、4/9，不代表完整规划全部完成。
